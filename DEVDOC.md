...
- **落地角度沒過完美/正立門檻時，改漸進給分、不再整包歸零**（2026-07-23）：`uprightCosThreshold`(0.55，≈57°內) 是滿分門檻；新增 `roughUprightCosThreshold`(0，≈90°內) 是「還算立著」的打折門檻，介於兩者之間依 `partialFlipRatio`(0.5) 打折給分，只有落地角度歪到接近側躺/倒插（cos ≤ 0）才 0 分。修的是「轉夠圈數但落地角度沒對齊，整趟翻轉分歸零」的挫折感（見 [constants.ts](src/game/constants.ts) RULES 區塊）。
