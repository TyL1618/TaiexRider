...
> **AdMob + Play Billing 兩座橋 2026-07-10 晚已完成程式碼串接＋本機 debug build 成功，
> 詳見下方「🌉 2026-07-10 晚：AdMob + Play Billing 兩座橋接完成」。⬜ 待真機側載驗證。**
...
capacitor-native-purchases／capgo-social-login）。**⬜ 尚未真機側載驗證**——Claude
的 preview 工具只能測瀏覽器頁面，Capacitor 原生功能（廣告是否真的顯示、Play Billing
彈窗、購買後鑽石是否入帳）必須使用者側載真機才能確認。
...
4. Google 登入：✅ 已裝 `@capgo/capacitor-social-login`、已串 `auth.ts`、真機驗證
   ...
   debug build 成功，詳見上方「🌉 兩座橋接完成」。⬜ 尚待真機側載驗證。
5. **優先驗證順序**（照這個順序做，已完成的部分打勾）：
   - ✅ Google 登入串接＋本機 build 成功
   - ✅ **真機側載驗證通過**：Credential Manager 系統選擇器登入成功，車庫金幣/車皮/
     暱稱跟 TWA 正式版完全一致，詳見上方「🎉 真機驗證結果」。
   - ✅ 程式碼串接＋本機 build：Play Billing、AdMob 獎勵廣告（詳見上方「🌉 兩座橋接
     完成」）。
   - ⬜ **下一步：真機側載驗證**——AdMob 三條路徑（車庫拿金幣/復活/結算雙倍）廣告
     是否正常顯示+發獎勵；Play Billing 購買鑽石包是否彈出原生付款視窗、付款後鑽石
     是否入帳（可用 Google Play 測試卡片，不會真的扣款——前提是這個 captest
     applicationId 有被加進 Play Console 的授權測試名單，否則 Billing API 可能查
     不到商品，見 `billing.ts` 檔頭「查價失敗」段落的常見原因）。
