...
| 🔴✅ 已修 | `submit_daily_score` 完全沒檢查「今天有沒有消耗過排名賽次數」——可繞過 UI 直接打 API、每 10 秒洗一次物理上限內的分數；每日排行榜名次直接發鑽石（真錢購買的付費貨幣），這條洞等同免費印鑽石 | `supabase/migration_20260711c.sql` 加 `wallet_daily_attempts.attempts ≥ 1` 檢查（同一 session），其餘既有物理一致性檢查（A1~A5）不變。詳見 [ANTICHEAT_DESIGN.md](ANTICHEAT_DESIGN.md) 2026-07-11 追加段落。 |

**殘餘已知風險（維持既有結論，非本輪新增）**：`wallet_earn` 系列在有伺服器每日上限封頂前提下仍可被「不玩只呼叫 RPC」薅到上限、`submit_daily_score` 仍未做「提交次數 ≤ 消耗次數」逐筆對帳（同一場次仍可在物理上限內反覆改進提交）（皆屬 ANTICHEAT Phase B 待正式上架後處理）；退款不自動收回鑽石（Voided Purchases API，同上）。
