...
（2026-07-09 曾嘗試寫一支 `sync-android-studio.ps1` 腳本做這件事，後來覺得路徑寫死在腳本裡
不夠彈性、不如直接讓 Claude 掃描找到實際路徑再動手，改成上面這種文字指引，腳本已移除。）
...
- **vc17**：做下面這項「AdBridgeService 改成只在看廣告時才短暫存活」。
- **vc18**：做「殼版本更新提示」——設計已在 DEVDOC.md §9.5b（方案 A：manifest `DEFAULT_URL`
  加 `?shell=N` 查詢參數 + Supabase `app_config` 表比對，版本落後就彈窗提示更新），
  2026-07-07 曾決定「不必單獨重包，等下次任何原因需要重包 AAB 時順便做」，現在 vc17
  剛好就是那個「下次」，排進 vc18 一起做。

### 🐛 2026-07-09（追加）：排行榜第 3~5 次「看廣告開始」從未真的觸發廣告 + 離開賽道警告文字補強

使用者真機測試排行榜第 3~5 次挑戰，發現按鈕雖顯示「看廣告開始」，卻直接跳過廣告進遊戲。
查證發現 [DailyChallenge.tsx](src/screens/DailyChallenge.tsx) 的 `showAd` 變數（第 3~5 次為
true）**從一開始就只拿去改按鈕文字/樣式，`handleStart()` 從未呼叫過 `requestRewardedAd()`**
——這個檔案的 git 歷史上從來沒有 import 過 `ads.ts`，比 2026-07-09 稍早那次修好復活/拿金幣
廣告橋接的提交（`dcdced7`）更早就存在，是半成品，不是這次改壞的。修法：`handleStart` 判斷
`showAd` 為真時，先 `await requestRewardedAd("coin")`，沒看完/失敗直接 return（不消耗次數、
不進遊戲），看完才照原本流程呼叫 `consumeAttemptServer()`；新增 `watchingAd` state 顯示
「廣告播放中…」並鎖按鈕避免重複點擊。

同時使用者指出：排行榜模式按「返回主選單」時，離開賽道確認彈窗（[GameCanvas.tsx:1548](src/game/GameCanvas.tsx:1548)）
文字只寫「目前成績不會保存」，語氣太輕——排行榜次數其實是在**按下開始的當下**就已經
`consumeAttemptServer()` 扣掉了（不是離開時才扣，中途離開也不會多扣一次，但也沒有任何
退還機制），彈窗完全沒提到這件事，玩家容易在不知情的狀況下白白浪費一次今天僅 5 次的機會。
改成 `analyticsMode === "daily"` 時顯示「本次挑戰次數將直接作廢，不會歸還」，其他模式維持
原文字不變。

純網頁改動，typecheck 過，preview 驗證首頁/排行榜畫面渲染正常、console 零錯誤。**⚠️ 廣告
橋接的 `requestRewardedAd` 在網頁環境會直接 resolve(true)（見 ads.ts），preview 無法測到
...
（前景服務常駐通知「TaiexRider・廣告服務待命中」全程都在），這是為了保險（怕漏接廣告請求）
才選的做法。使用者反應比較喜歡原本沒有常駐通知的乾淨畫面。

**改善方向**：服務其實只需要在「使用者按下看廣告」到「廣告播完」這段期間存活，平常玩遊戲
用不到。改法：
1. `MainActivity.kt` 移除啟動 `AdBridgeService` 的呼叫（保留通知權限請求 + `shouldLaunchImmediately()`
   延後啟動的邏輯不動，這樣才能讓使用者在自然的開機時機回答權限，之後看廣告時已經有結果）。
2. `AdActivity.kt` 原本就有的 `startForegroundService()` 呼叫變成唯一的啟動點。
3. `AdBridgeService.kt` 加自動關閉邏輯：`/ad/reset` 進來時（新一輪看廣告開始）用
   `Handler.postDelayed` 設一個保底逾時（例如 120 秒，避免流程卡住通知永遠留著）；
   `/ad/result` 偵測到 `done` 第一次變 `true` 時，取消保底逾時、改成短延遲（例如 8 秒，
   讓網頁端輪詢有時間抓到結果）後呼叫 `stopForeground()` + `stopSelf()`。
4. 這樣通知只會在「點看廣告→廣告播完後幾秒」這段期間短暫出現，平常玩遊戲/App 剛開啟時
   通知欄乾淨，符合 Android 官方建議的「前景服務只在真的做事時才顯示通知」原則。

**不能做的事**：前景服務**一定要**顯示通知，這是 Android 系統設計上刻意的透明度要求，
無法讓前景服務完全不顯示任何通知——只能縮短它「存在」的時間，不能讓它存在卻不顯示。

### 🐛 2026-07-09（九度追加）：vc15 上傳 Play Console 卡在前景服務用途聲明，順手抓到通知從未顯示的 bug

vc15 上傳 Play Console 封測軌道時，Google 要求填「前景服務用途聲明表單」——這是
`FOREGROUND_SERVICE_SPECIAL_USE` 這個權限類型的必經審查步驟（規劃階段就預期到，見
DEVDOC.md §9.4c 最後一段）。填寫過程中使用者回報「通知欄一直都沒有出現過那個常駐通知」，
**抓到一個真正的疏漏**：`AdBridgeService` 雖然正確呼叫了 `startForeground()`，但**從頭到尾
沒有跟使用者要過 `POST_NOTIFICATIONS` 這個 Android 13+ 執行時權限**——前景服務本身不受
影響（廣告功能一直正常，優先權跟通知顯示與否是分開的兩件事），但系統會靜默不顯示通知，
導致玩家/Play Console 審查都看不到「TaiexRider・廣告服務待命中」。

**修法**：`MainActivity.kt` 的 `onCreate()` 加上 API 33+ 判斷式的 `ActivityCompat.
requestPermissions(POST_NOTIFICATIONS)`（`LauncherActivity` 繼承的是普通
...
但廣告功能不受影響）。**⚠️ 十一度追加（真正的根因）**：重新測試發現「權限對話框閃現不到一秒就消失，
根本來不及按」——使用者強制去系統「App 資訊」手動開啟通知後，確認通知本身能正常
顯示（`AdBridgeService` 的通知建構程式碼沒問題），問題全部出在「對話框沒時間讓
使用者互動」。**真因**：`LauncherActivity.onCreate()` 本身在 `super.onCreate()`
呼叫當下就會自動立刻啟動 TWA 瀏覽器畫面，跟我們的權限對話框搶 Activity 焦點——
瀏覽器畫面幾乎立刻蓋上來，把權限對話框直接蓋掉。**修法**：改用 `LauncherActivity`
本來就提供的擴充點——覆寫 `shouldLaunchImmediately()` 回傳 `false`（需要問權限時）
延後自動啟動 TWA，等 `onRequestPermissionsResult()` 回呼、權限狀態確定後，才手動
呼叫 `launchTwa()`（`protected` method，可從子類別呼叫）補上原本被延後的啟動。
副作用：需要問權限的那一次（通常是全新安裝後第一次開啟），使用者會先看到短暫的
空白/透明畫面（原本立刻出現的 splash 品牌圖也一起延後了）才是權限對話框，回答後
才接著看到 splash→TWA，是可接受的一次性小瑕疵。**待 vc16 重新建置測試**。

### 🔴🎉 2026-07-09（八度追加，真因）：AdBridgeService 缺 CORS 標頭，網頁端永遠讀不到伺服器已經正確回應的結果

visibilitychange 修好後，使用者回報「完全一樣，還是不管復活/雙倍/拿金幣都不會發獎勵，
按鈕卡在『廣告播放中』快 1 分鐘或切頁面才恢復」。用 adb 指令同時篩選 `AdBridgeService`/
`AdActivity` 兩個 tag、在同一時間軸完整記錄一次測試，抓到決定性證據：

```
onAdDismissedFullScreenContent rewardEarned=true   ← 廣告正確關閉
GET /ad/result -> done=true granted=true           ← 伺服器正確回應（此後持續 7+ 次都是 true）
```

伺服器端從頭到尾都是對的，但網頁端從未真的「看到」這個 true，卡滿 60 秒逾時
（`AD_BRIDGE_TIMEOUT_MS`）才放棄回傳 false——這個「卡快 1 分鐘」跟 60 秒逾時完全吻合，
才是問題全貌：**輪詢邏輯本身沒有錯，是回應根本沒有真的送達網頁 JS 手上**。

**真因＝CORS**：網頁來源 `https://taiexrider.pages.dev` 跟 server `http://127.0.0.1:47591`
scheme/host/port 全部不同，屬於跨來源請求。`AdBridgeService` 的回應完全沒有加
`Access-Control-Allow-Origin` 標頭——瀏覽器仍然會把請求送出去、伺服器仍然正確處理並
回應（這就是為什麼 Android 端 log 一直顯示 `done=true` 且看起來一切正常），**但瀏覽器
會擋住網頁 JS 讀取那個回應內容**，`fetch()` 在網頁端直接視為失敗，被我們的 catch 區塊
默默吞掉、繼續下一輪重試，永遠不會真的走到 `if (data.done) return ...` 那一行。這完美
解釋了先前排查時「已經下載部署好的程式碼逐行比對、邏輯確認正確」卻仍然重現的矛盾——
程式碼真的沒錯，錯在少了一個 HTTP 標頭。
...
加上 `AdActivity` 自己也啟動 `AdBridgeService` 後真機重測，`onAdDismissedFullScreenContent
rewardEarned=true` 觸發後幾乎立刻（<1s）就看到 `GET /ad/result -> done=true granted=true`
——**整條「網頁→原生→顯示廣告→回報結果」橋接確認打通**，前六輪踩過的坑（PostMessage
死路、Service 被背景省電殺掉、BAL 擋下 startActivity、離開網站確認框、AppCompatActivity
主題崩潰、行程被系統砍掉重開）全部解決。

但發現新問題：伺服器端 `done=true granted=true` 之後**持續被問了 13 秒**（`/ad/result`
重複回應同樣結果 26 次）才停止，且使用者回報**廣告播完後獎勵完全沒入帳、按鈕卡在
「廣告播放中…」**，要切到別的畫面再切回來才「看起來正常」——但那不是真的修好，是卡住
的 `GameCanvas` 元件被整個卸載重新掛載，剛好變回預設狀態（連帶次數也沒被消耗，等於
白看一次廣告）。**根因**：`AdActivity` 全螢幕蓋在畫面上時，原本顯示遊戲的 TWA 分頁對
Chrome 來說變成「背景分頁」，Chrome 會把 `setTimeout`/`setInterval` 大幅節流（背景夠久
會節流到每分鐘最多執行一次）——我們的輪詢迴圈用 `setTimeout` 實作，導致就算伺服器端
早就準備好結果，網頁端的計時器也要等被節流後才會醒來檢查，使用者回報「要等好幾分鐘」
正好吻合 Chrome intensive timer throttling 的行為特徵。

**修法**：`src/lib/ads.ts` 新增 `waitOrWake()` 取代原本單純的 `sleep()`——同時監聽
`visibilitychange` 事件，分頁一恢復可見（廣告關閉、焦點回到 TWA 的瞬間）就立刻醒來
檢查一次，不受計時器節流影響；原本的固定間隔（500ms）保留當一般情況下的節奏，只是
不再是唯一喚醒管道。純網頁改動，不需要重新建置 android/，push 後等 CI 部署完直接在
手機上重開遊戲測試即可。**待真機重新測試確認獎勵能即時入帳**。

### 🔴 2026-07-09（六度追加）：真因＝載入廣告時系統把整個 App 行程砍掉重開，新行程沒有 AdBridgeService

加 log 後真機重測，`onAd` 篩選看到關鍵證據：

```
---------------------------- PROCESS ENDED (30524) ----------------------------
---------------------------- PROCESS STARTED (31923) ----------------------------
AdActivity  onAdLoaded, showing now              ← pid 31923
AdActivity  onAdDismissedFullScreenContent rewardEarned=true   ← pid 31923
```

`/ad/` 篩選則完全沒有任何一行——代表 `AdBridgeService` 的 `serve()` 一次都沒被呼叫到。
兩者合起來：**載入廣告影片很吃記憶體，系統把整個 App 行程（原本跑著已確認成功啟動的
`AdBridgeService` 那個行程，pid 30524）整個砍掉重開**，Android 為了處理待處理的
`AdActivity` intent，用全新行程（31923）直接冷啟動這支 Activity——**這個新行程從頭到尾
沒有經過 `MainActivity.onCreate()`**（那是原本唯一會呼叫 `startForegroundService`
啟動 `AdBridgeService` 的地方），廣告在新行程裡順利載入、顯示、關閉、`rewardEarned=true`
都正確記錄下來，但這個新行程裡**根本沒有 server 在監聽 127.0.0.1:47591**，網頁端所有
`/ad/reset`／`/ad/result` fetch 全部連不上（連不上不會報 log，這解釋了「/ad/ 沒東西」）。

**修法**：`AdActivity.onCreate()` 也自己呼叫一次
`ContextCompat.startForegroundService(this, Intent(this, AdBridgeService::class.java))`
（`AdBridgeService.onCreate()` 有 `if (server != null) return` 保護，重複啟動是安全的
——已經在跑的話只是多送一個 `onStartCommand`，不會重複建立 server）。這樣不管廣告最後
是在原本行程播、還是系統砍掉重開的新行程播，這個行程都保證有 server 在聽，網頁端輪詢
才問得到結果。**待真機重新測試**——這是目前為止最根本的一個坑，如果修好了，前面五輪
（BAL、離開網站確認框、AppCompatActivity 崩潰、行程被砍）應該就是完整的問題鏈全部
解開，廣告橋接整條應該可以跑通。

### 🟡 2026-07-09（五度追加）：改繼承 Activity 後廣告真的能顯示了，但關閉後網頁端卡在「廣告播放中」不給獎勵，加 log 診斷

改成 `Activity`（不再 `AppCompatActivity`）後真機重測，**廣告終於正常顯示、能完整看完**
（比之前進步一大步）。但使用者回報：廣告關閉後（按下 2-3 秒黑畫面後跳出的 Google 測試
廣告的關閉鈕），網頁按鈕仍卡在「廣告播放中…」不會變化，也沒有發放獎勵——代表輪詢
（`pollAdResult()`）沒有正確拿到 `done:true` 的結果，或 `AdActivity` 的
`onAdDismissedFullScreenContent` 沒有被正確觸發，但目前完全沒有 log 可以判斷卡在
哪一段。**追加診斷 log**（`AdBridgeService.serve()` 每次 `/ad/reset`／`/ad/result` 都印
一行；`AdActivity` 的 `onAdLoaded`／`onUserEarnedReward`／`onAdDismissedFullScreenContent`
／`onAdFailedToShowFullScreenContent`／`onAdFailedToLoad`／`onDestroy` 全部印 log；
`ads.ts` 也補上輪詢開始/結束/逾時的 console log），**下次真機測試時把 Logcat
（篩選 `Ad`）+ Chrome DevTools console 一起截圖回報，才能精準定位卡在原生端還是網頁端**。

### 🔴 2026-07-09（四度追加）：window.open 修好了！但 AdActivity 啟動瞬間崩潰——AppCompatActivity 配錯主題

`window.open` 修好後真機重測，這次**確認自訂 scheme 觸發本身成功了**——logcat 看到
`onActivityEvent Snapshot Destroyed: ComponentInfo{.../.AdActivity}`，代表 `AdActivity`
真的有啟動（不再 BAL_BLOCK，也沒再跳「離開網站」確認框）。但緊接著出現
`com.samsung.android.lool/...AppErrorDialogActivity`（三星 Device Care 的應用程式錯誤
對話框），代表 `AdActivity` 啟動瞬間就當機，使用者關掉這個系統錯誤對話框後回到桌面，
感覺上像是「遊戲被關掉了」。

**根因**：`AdActivity` 繼承 `androidx.appcompat.app.AppCompatActivity`，但 manifest 給的
`android:theme` 是 `@android:style/Theme.Translucent.NoTitleBar`（純 Android 系統主題，
不是 `Theme.AppCompat` 系列）——這個組合是 Android 開發很經典的地雷，`AppCompatActivity`
強制要求 AppCompat 系主題，配錯會直接拋 `IllegalStateException` 崩潰。這支畫面本來就沒有
工具列/Fragment，不需要任何 AppCompat 功能，**改成繼承普通的 `android.app.Activity`**
解決（`RewardedAd.show()` 只需要 `Activity` 參數，不需要 `AppCompatActivity`）。
**待真機重新測試**。

### 🔴 2026-07-09（三度追加）：自訂 scheme 觸發跳出「要離開網站嗎」，改用 window.open 避免導離 TWA 本體

改用 `<a>.click()` 導轉自訂 scheme 後真機重測，點按鈕跳出 Chrome「要離開網站嗎」確認框
（只有取消/離開兩個按鈕），按「離開」**直接把整個 TWA 關掉**，不是只是去開廣告畫面而已。
根因：`<a>.click()` 是在**同一份文件**內導轉，TWA 對「離開目前受信任的網站」有額外保護
（見 Chrome TWA 文件「Leaving the Trusted Web Activity」），會先跳確認框，使用者按離開
後 Chrome 判定失去信任來源、直接退出整個 TWA fullscreen 體驗——這正是為什麼「按離開遊戲
就關掉」。

**修法**：`src/lib/ads.ts` 的 `requestTwaRewardedAd()` 改用 `window.open('taiexrider-ad://...', '_blank')`
取代 `<a>.click()`——開一個新的瀏覽情境去嘗試導轉，原本這份 TWA 文件本身不導航、不會
觸發「離開」保護機制，新視窗解析到系統交給 Android 處理自訂 scheme（沒有網頁內容可顯示）
即結束。**待真機重新測試**——如果這個方法仍然跳出確認框或有其他 Chrome 版本相依的怪異
行為，下一步考慮改回原生端做法（`setFullScreenIntent()` 通知，或退回 CLAUDE.md 早先
記錄的「WebView 專屬畫面」保底方案，但兩者都有各自的 Play 政策/工程量顧慮，先試這個
最小改動）。

### 🔴 2026-07-09（再追加）：改前景服務仍卡住——真因是 Background Activity Launch 限制，改用自訂 URL scheme 觸發

改前景服務後真機重測（Logcat 確認 `AdBridge server started on 127.0.0.1:47591` 且
`Background started FGS: Allowed`，server 這次真的活著），點「觀看廣告 獎勵 ×2」仍然
沒反應。Logcat 抓到真正關鍵一行：

```
ActivityTaskManager  I  START ... cmp=com.tylapp.taiexrider/.AdActivity ...
                        (BAL_BLOCK) result code=102
```

`BAL_BLOCK`＝`AdBridgeService` 收到 HTTP 請求後呼叫 `context.startActivity(AdActivity)`
被 Android 的 **Background Activity Launch（BAL）限制**直接擋下。查證 Android 官方文件
「Restrictions on starting activities from the background」列出的完整豁免清單（App 有
可見視窗／是目前的輸入法／由系統送出的 PendingIntent（如通知點擊）／有
`SYSTEM_ALERT_WINDOW` 權限／有 `START_ACTIVITIES_FROM_BACKGROUND` 權限／被有背景啟動
權限的 Service bind／由桌面 launcher 發起／由必須全程執行的系統核心元件發起）——
**前景服務完全不在名單內**，跟一開始規劃時想的不一樣，這不是「記憶體吃緊才會發生」的
偶發問題，是 OS 層級寫死的安全限制，改哪種 Service 類型都沒用。

**修法（架構性調整）**：唯一可靠的豁免是「由目前可見的前景 App 發起」——把「誰觸發
`AdActivity`」這件事，從「Service 收到 HTTP 請求後自己開」改成「網頁按鈕點擊當下，
用使用者手勢直接導向一個自訂 URL scheme（`taiexrider-ad://show?type=coin|revive`），
讓 Chrome（前景、可見）幫我們開」——Chrome 是發起者，不受 BAL 限制。`AdBridgeService`
降級成單純的「結果暫存區查詢站」（`/ad/reset` 清空、`/ad/result` 回報目前狀態），
不再呼叫 `startActivity`。網頁端（`ads.ts`）改成：按鈕點擊同步建立 `<a>` 導轉自訂
scheme（必須跟使用者手勢留在同一個同步呼叫堆疊內，Chrome 才會放行，不會被當背景彈出
擋掉）→ 之後每 500ms 輪詢 `/ad/result` 直到有結果，逾時 60s（原本設 10~12s 逾時，
順便發現這對一支真正 15~30s 的獎勵廣告影片來說太短，不管 BAL 擋不擋都會先逾時失敗，
這次一併修正）。`AdActivity.kt` 改讀 `intent.data` 的 `type` query 參數（原本讀 Intent
extra，因為現在由 URL 導轉啟動，不再有機會塞 extra）；manifest 幫 `AdActivity` 加
`android:exported="true"` + 自訂 scheme 的 intent-filter。**待真機重新測試**。

### 🐛 2026-07-09（真機實測追加）：AdBridgeService 被系統背景省電機制停掉，改前景服務

vc15 debug build 真機（Samsung One UI，Android 16）實測，點「觀看廣告 獎勵 ×2」卡在
「廣告播放中…」不動、什麼都沒跳出來。Logcat 抓到關鍵一行：
`ActivityManager  W  Stopping service due to app idle`——`MainActivity`（LauncherActivity）
啟動 TWA 成功後很快 `finish()`，App 對系統來說變成「背景無互動」，**三星 One UI 在幾分鐘內
就把普通（非前景）Service 判定 app idle 停掉**，`AdBridgeService` 死掉後，網頁 `fetch()`
送到 127.0.0.1 沒人接，卡到逾時。這正好印證了規劃階段就評估過的風險（「Service 沒做成
前景服務，系統記憶體吃緊時有機會被殺掉」），只是沒想到不用等記憶體吃緊、幾分鐘內就在
三星機型上發生。

跟使用者確認後選擇修法：**`AdBridgeService` 改成前景服務**（`startForeground()` +
常駐通知，foregroundServiceType 用 `specialUse`——沒有其他官方類型剛好符合「本機
loopback server 橋接廣告」這種用途，Android 官方文件也建議這種情況才用 specialUse）。
代價是 App 開著的時候手機通知欄會有一個常駐通知「TaiexRider・廣告服務待命中」，換取
server 不會被背景省電機制提早殺掉。`MainActivity` 啟動服務的呼叫從 `startService()`
改成 `ContextCompat.startForegroundService()`（API 26+ 要求前景服務用這個入口啟動，
否則 5 秒內沒呼叫 `startForeground()` 系統會拋例外）。**待重新真機測試確認 server
是否還會被停掉、廣告是否能正常跳出**。

備援方案（如果前景服務仍不夠、或使用者覺得常駐通知太干擾）：換成「自訂 URL scheme +
manifest intent-filter」觸發，不需要任何背景常駐、網頁按鈕點擊時導向
`taiexrider-ad://show` 讓系統直接喚醒/冷啟動 App 處理，但要重寫「JS 怎麼通知原生」
這一段（`AdActivity`/`AdBridge` 顯示廣告的部分不用動），工程量較大，先不做。

### 🎯 2026-07-09（晚間）：AdMob 獎勵廣告橋接動工——PostMessage 主線方案證實走不通，改用本機 loopback HTTP server（vc15）

早上規劃時記錄的「PostMessage for TWA 主線方案」深入查證後發現一個沒被發現的坑：
Chrome 官方範例的 MainActivity **不繼承** androidbrowserhelper 的 `LauncherActivity`，
自己手刻 `CustomTabsClient` 綁定+session+啟動 TWA，因為 postMessage 需要「啟動 TWA 那個
`CustomTabsSession`」的直接參照。我們的 `MainActivity` 繼承 `LauncherActivity`（要留著
splash/shortcuts/預測性返回），它內部的 `TwaLauncher` 把這個 session 存成 **private 欄位、
無 getter**，而且 `LauncherActivity` 啟動 TWA 成功後**一定會呼叫 `finish()`**（設計上是
trampoline，不留在背景）——代表就算硬挖到 session，Activity 當下也已經不在了。查到
GitHub [issue #472](https://github.com/GoogleChrome/android-browser-helper/issues/472)
是另一個用 Bubblewrap（跟我們專案結構一樣）的開發者踩到一模一樣的牆，2026-07 查證時仍
open、無解，結論是「除非改函式庫原始碼，沒有乾淨做法」。

跟使用者討論四個方案（本機 loopback HTTP server／reflection 挖 TwaLauncher 私有欄位／
fork TwaLauncher.java 加 getter／WebView 專屬畫面）的可靠度與可替換性後，**選定本機
loopback HTTP server**：網頁 JS `fetch('http://127.0.0.1:47591/ad/rewarded?type=coin|revive')`
（loopback 對 HTTPS 頁面不算 mixed content），完全不碰 TWA session/postMessage，`AdBridgeService`
（`android/app/src/main/java/com/tylapp/taiexrider/AdBridgeService.kt`，NanoHTTPD）由
`MainActivity.onCreate()` 啟動、獨立於 LauncherActivity 存活（started service 不會因為
啟動它的 Activity finish() 而跟著死），收到請求後透過 `AdBridge.kt` 啟動透明的
`AdActivity.kt` 實際載入/顯示 `RewardedAd`（目前用 Google 官方測試單元 ID，不受 AdMob
帳戶審核狀態影響，上架前才需換成真實單元 ID），結果經同一個物件傳回、擋著 HTTP 請求
直到有結果才回應。四個方案都共用同一個網頁端呼叫介面（`ads.ts` 的
`requestRewardedAd(kind)`），之後如果這個方案測不出來，換其他方案不用動遊戲邏輯。

同時發現並修復一個真的 bug：**「看廣告復活」按鈕從以前到現在點下去都是直接免費復活，
從沒真的呼叫過任何廣告函式**（`GameCanvas.tsx` 原本的 onClick 只有
`setRevivalUsed(true); requestRevive();`），跟按鈕文字（已購買永久去廣告才顯示「復活」）
不符——補上 `handleWatchAdRevive()` 真的呼叫 `requestRewardedAd("revive")`，沒看完/失敗
就不消耗復活機會。

改動：`android/app/build.gradle.kts`（加 `play-services-ads:25.4.0` +
`nanohttpd:2.3.1`，versionCode 14→15）、`AndroidManifest.xml`（INTERNET/
ACCESS_NETWORK_STATE 權限、AdMob `APPLICATION_ID` meta-data、註冊 `AdBridgeService`/
`AdActivity`）、新增 `AdBridge.kt`/`AdBridgeService.kt`/`AdActivity.kt`、
`MainActivity.kt`（`onCreate` 啟動 Service）、`src/lib/ads.ts`（`requestRewardedCoins()`
改名 `requestRewardedAd(kind)`，TWA 環境走 loopback fetch、網頁環境維持 stub 直接發獎勵）、
`GameCanvas.tsx`/`Garage.tsx`（呼叫端更新）、`public/_headers`（CSP connect-src 補
`http://127.0.0.1:47591`）。typecheck 過，preview 驗證訪客路徑零回歸（手動步進物理跑完
一場、console 零錯誤；死亡結算面板因 preview 隱藏分頁 rAF 限制沒能實際點到復活按鈕，
與 GameCanvas 一貫的已知限制相同）。

**⚠️ android/ 部分 push 不會生效，且這次是全新的橋接機制，需要使用者手動：**
1. Android Studio 開啟 `C:\Users\tyl16\AndroidStudioProjects\TaiexRider\`（五個檔案
   已同步複製過去），Sync Gradle（會抓 play-services-ads + nanohttpd 兩個新依賴）。
2. **先用 debug build + `adb install` 快速測試**，不用每次都走 Generate Signed
   Bundle 上傳 Play Console 那套完整流程——loopback server/AdActivity 跟 TWA 網址
   驗證無關，debug build 裝置本機測試就能看到本機 server 是否啟動、廣告是否顯示、
   `console.error` 是否印出橋接失敗原因。
3. 真機測試重點：進遊戲摔車後點「看廣告復活」、結算畫面點「觀看廣告 獎勵 ×2」，
   應該會跳出 Google 測試廣告（畫面會有明顯的「Test Ad」字樣，這是正常的，不是真的
   廣告單元），關閉/看完後應該正確復活/雙倍金幣。若按鈕卡著沒反應或一直失敗，
   看 `adb logcat` 抓 `AdBridgeService`/`AdActivity` tag 的 log，或瀏覽器
   devtools console 抓 `[ads]` 開頭的錯誤訊息。
4. 確認整條路徑通了之後，才需要 Generate Signed Bundle 上傳 Play Console 封測軌道
   （vc15），這步可以晚一點、等真機驗證過再做。
5. 上架前記得把 `AdActivity.kt` 的 `TEST_REWARDED_AD_UNIT_ID` 換成真實單元 ID
   （revive_reward: `ca-app-pub-8981745966447649/1679422480`；coin_reward:
   `ca-app-pub-8981745966447649/2170377077`）。

### 🔴🎉 2026-07-09（追加）：鑽石購買「暫無法購買」真正根因＝TWA Billing 橋接元件從未接上（v0.12.44，vc12）

昨晚（見上方 2026-07-07 段落）以為只是授權測試名單/商品剛生效延遲，結果排查一整輪
（授權測試清單 14 人全勾、商品 4 個皆有效、BILLING 權限已在 vc11 上線、assetlinks.json
SHA-256 跟 Play App Signing 憑證完全比對相符、付款設定檔正常）**全部正常**，仍然所有
測試人員都卡在灰色「暫無法購買」。用 `adb logcat` 直接連真機（一般 chrome://inspect
連不上——release 簽名版預設不給 Chrome 暴露 TWA 內容給 DevTools 檢查，release 版一律連
不上，這是 Chrome 的安全限制不是設定沒開好）抓到關鍵一行：
`cr_DigitalGoods: Unable to execute getDetails.`。

查 Chrome 官方文件（developer.chrome.com「Use Play Billing in your Trusted Web
Activity」）比對現有 manifest，找到真正根因：**2026-07-06 那次加 Play Billing 只做了
一半**——只加了 Gradle 依賴（`androidbrowserhelper:billing:1.1.0`）跟 `BILLING` 權限，
但 androidbrowserhelper 這個依賴**不會自動接上任何東西**，還需要三塊完全沒做的設定：
1. 自訂 `DelegationService` 子類別（繼承函式庫預設版，在 `onCreate()` 呼叫
   `registerExtraCommandHandler(DigitalGoodsRequestHandler(...))`）——單純用函式庫
   預設的 `DelegationService` 不會有任何 Digital Goods 處理能力。
2. 這個 service 的 manifest 宣告要加 `intent-filter`（action
   `android.support.customtabs.trusted.TRUSTED_WEB_ACTIVITY_SERVICE`）——原本完全沒
   宣告 intent-filter，Chrome 根本找不到可以處理請求的對象。
3. `PaymentActivity`（action `org.chromium.intent.action.PAY`）+ `PaymentService`
   （action `org.chromium.intent.action.IS_READY_TO_PAY`）這兩個 androidbrowserhelper
   專屬付款橋接元件——manifest 裡原本只有 Play Billing Library 自帶的
   `ProxyBillingActivity`/`ProxyBillingActivityV2`，跟這兩個完全不是同一組、不能互相
   替代。

**已修復**：新增 [android/app/src/main/java/com/tylapp/taiexrider/DelegationService.kt](android/app/src/main/java/com/tylapp/taiexrider/DelegationService.kt)
+ [AndroidManifest.xml](android/app/src/main/AndroidManifest.xml) 補上述三塊設定 +
versionCode 11→12（1.11→1.12）。已同步複製進 Android Studio 專案
（`C:\Users\tyl16\AndroidStudioProjects\TaiexRider\`）。**⚠️ 待辦：Android Studio
重新建置 → Generate Signed Bundle → 上傳 Play Console 封測軌道（vc12）→ 等測試人員手機
...
BILLING 權限（vc11 manifest 已含，且 vc11 確認是 Play Console 目前上線的版本）、付款
設定檔（銀行帳戶已綁定，無待驗證警示）、assetlinks.json 的 SHA-256 指紋（跟 Play
App Signing 憑證完全一致）、手機端安裝版本（確認已更新到 vc11）。

### 🐛 2026-07-09：events.player_id 從 7/2 上線以來全部 NULL（分析事件打點的登入身分沒帶到）

使用者想查特定帳號（王柏翰兩個帳號）「最後一次玩是什麼時候」，用 `player_id` 查
`events` 表全部 0 row，連使用者自己的 UID 也查不到——一開始懷疑是那兩個帳號真的沒玩過，
但直接用 SQL 驗證 `select count(*), count(player_id) from events` 得到 `377 / 0`，
證實**這欄從 7/2 上線以來對所有人都是 NULL**，不是特定帳號的問題。

**根因**：[analytics.ts](src/lib/analytics.ts) 的 `logEvent()` 打 `log_event` RPC 時，
`Authorization` header 固定用 `VITE_SUPABASE_ANON_KEY`（公用金鑰，不代表任何登入者），
而伺服器端 `player_id = auth.uid()::text`（[migration_20260702.sql:46](supabase/migration_20260702.sql:46)）
是靠請求帶的 token 判斷身分——用 anon key 打過去，`auth.uid()` 永遠是 NULL。`device_id`
（localStorage 裝置 UUID）這欄沒受影響、一直是對的，但**「device_id ↔ 帳號」這層關聯
從一開始就沒有在任何地方被正確記錄過**（`getPlayerId()` 整個專案只有 analytics.ts 這裡
用到，沒有其他表把它跟正確身分一起存過），所以過去的 377 筆事件**無法回溯修補**，只能
靠跟其他表（`daily_scores.created_at`/`player_wallet.updated_at`）的時間點對照做間接推測。

**修法**：`logEvent()` 改成先 `supabase.auth.getSession()` 讀目前 session，登入時帶
真正的 `access_token`，訪客才 fallback 用 anon key（訪客本來就該是 NULL，這樣才對）。
保留原本的 fire-and-forget（不 await 外層）+ `keepalive:true`（頁面關閉/切走時盡量送出）
不變。typecheck+build 皆過，純網頁改動，隨 push 自動部署。**修完只影響「之後」的新紀錄**，
之後才能真正用 `player_id` 查特定帳號的遊玩史。

### 💰🛡️ 2026-07-09（追加）：IAP 金流鏈路完整稽核 + 補上兩個真錢缺口（v0.12.45）

使用者要求「不只按鈕會亮，整個購買流程不能出錯，牽扯金流」，逐段稽核前端付款 →
Edge Function 驗證 → DB 發放/防重放整條鏈路。**核心防護本來就穩**（伺服器驗證不可繞過、
`iap_purchases` purchase_token 防重放、只認 purchaseState=0、金額以伺服器為準、鑽石
consume／去廣告 acknowledge 分流正確、`grant_iap_diamonds` 的 SQL 歧義 bug 已於
migration_20260709b 修好、`grant_remove_ads` 結構上無此 bug），但揪出**兩個會造成真錢
損失/卡單的缺口，已一併修復**：

- **缺口 1（真錢損失）**：Edge Function 的 `consume`/`acknowledge` 原本用 `.catch(()=>{})`
  靜默吞掉失敗，卻照樣回 `ok:true` 發鑽石。Google 規定 3 天內沒 consume/acknowledge 會
  **自動退款** → 「鑽石已發 + 又被退款」＝直接虧損；且 token 已記進 iap_purchases，重試會
  走防重放分支提早 return，永遠補不了 consume。**修法**：`consumePurchase`/
  `acknowledgePurchase` 改回傳 bool（400 alreadyConsumed/alreadyAcknowledged 視為成功＝
  冪等），失敗就回 `ok:false`（502 pending）不回報成功；主流程改成「已發過(replay)也算
  成功狀態、每次都往下重試 consume/acknowledge」，只有真失敗才不回報成功。
- **缺口 2（付款成功但發放中途失敗 → 錢卡住）**：付款完成後若 session 遺失/function 逾時，
  前端當作沒買到、什麼都不發，且全站**沒有 listPurchases 對帳機制**，玩家錢扣了鑽石沒進、
  要等 3 天退款。**修法**：`billing.ts` 新增 `reconcilePurchases()`（進車庫時撈
  `listPurchases()` 把「已扣款但沒發成功」的孤兒交易重送 Edge Function 補發，已成功對帳的
  token 記在本地 `tr_iap_reconciled` 避免每次重打），`Garage.tsx` 掛載時呼叫、補到的餘額
  直接更新 UI。搭配缺口 1 的冪等化，孤兒交易下次進車庫就自動補發+補 consume，不必等退款。

typecheck + build 皆過。**⚠️ 待辦**：① Edge Function 改了程式碼，必須手動重新
`supabase functions deploy verify-iap-purchase` 才會生效（push 不會更新雲端 function）；
...
確認新的能用後可刪 **← 使用者已把 Downloads 全部清空，新金鑰 secret 已設好不受影響**；
② 若在意安全，Google 登入用的 OAuth client_secret 可 rotate（曾在對話中顯示過，封測期風險低，選配）。

### 🔒 2026-07-09（追加）：未登入訪客擋掉真錢購買（雙層防護）

IAP 打通後發現漏洞：車庫購買區塊原本只用 `billingAvailable` 判斷、**沒檢查登入**，訪客在
TWA 裡能按購買 → Google 真的扣款 → 但後端沒帳號可入帳 → 扣了錢拿不到鑽石（只能等 3 天退款），
比「本地錢包不保險」更糟。修法兩層：① `billing.ts` `runPurchaseFlow()` 在叫出 PaymentRequest
**之前**先查 session，未登入直接回「請先登入」不扣款（最關鍵）；② `Garage.tsx` 訪客不查價/
不對帳，「購買鑽石＋永久去廣告」區塊改顯示 🔒 登入提示、不給按鈕。**盤點結論：只有真錢 IAP
需要擋**——鑽石車款訪客 0 鑽石自然買不了、金幣車款/看廣告/成就車款純本地外觀無金錢或排行榜
影響（刻意的訪客沙盒）、排行榜賽 2026-07-07 已要求登入。純網頁改動，typecheck+build 過。

### 📋 2026-07-09：Testers Community 客服確認（封測 14 天倒數，放心等即可）

跟 Testers Community 客服往返釐清，重點結論（**申請正式版前務必再讀**）：
- **以他們的儀表板「14 天完成」為準去申請正式版**，不是看 Play Console 的天數（Play Console
  可能提早或誤顯示，都不影響）。
- **繼續用原本的測試群組**，只要確認他們的 Google 群組 `testers-community@googlegroups.com`
  已加入（已加）。
- **人數不夠的日子不計入 14 天、不會白算**，所以被朋友退出打回「1 天」不會浪費時間。
- Play Console「已安裝目標對象(Installed Audience)」出了名的不準/延遲（Google 2025 底自己
  掛過公告承認）；**真正有意義的是 opt-in 計數**——截圖「第二行被劃掉」＝已達 12 人。他們用
  內部 MDM 確認 25 位測試者都已安裝且每天開 App。
- 他們過去 6 個月 **100% 成功率**，14 天後正式版被拒**全額退款**。
- 「Notify us」按鈕是他們 bot 的 bug，改用 email 通知已 OK。
- **一句話**：設定都對、人數其實夠了，Play Console 數字只是延遲，耐心等他們儀表板到 14 天再
  申請即可，不用再焦慮盯人數。
...
2. **鑽石車款（P 系列，5 台）＋鑽石購買頁**：✅ **2026-07-07 五台全數生圖完成上線，2026-07-09 已重新用程式碼核實確認（`garage.ts`/`Garage.tsx`/`public/bikes/` 圖檔皆齊全），此項無待辦，不要再問**——P1赤紅暴走/P4電馭武士/P3黃金期貨（原黃金大亨）/P5匿蹤幽靈（原幽靈匿蹤）/P2銀河鍍鉻，車庫「鑽石車款」區塊皆可購買/裝備。**同日下午使用者拍板重新排序定價**（銀河鍍鉻改成最貴）：赤紅300／武士380／黃金期貨450／匿蹤幽靈520／銀河鍍鉻600（id 不變，IAP 真實定價待穩定後再決定）。**✅ `supabase/migration_20260707b.sql` + `migration_20260707c.sql` 皆已跑**（b 先補上 P3~P5 白名單，c 再重排定價，create-or-replace 疊加生效）。**✅ 真錢購買鑽石/永久去廣告（`diamonds_100/350/1200`/`remove_ads_forever` IAP）2026-07-09 已完全打通並真機實測通過**（購買→入帳→ads_removed 皆正常），完整排查過程與六層問題鏈見上方「2026-07-09（收尾）」段落。目前售價 TWD 31/94/280/72（顯示已修正去除多餘小數）。**未登入訪客已擋購買**（雙層防護，見「2026-07-09（追加）：未登入訪客擋掉真錢購買」段落）。
...
4. **RETENTION_PLAN 第三批**（週聯賽分組、Ghost 回放）：長期規劃，工程量大，未排入近期；Ghost 回放依規劃需跟反作弊第四層一起設計，屬 Fable 5 範圍。~~好友邀請比較~~、~~週五馬拉松~~、~~排行榜 emoji 反應~~ 使用者決定取消（好友邀請比較沒有實際需求、週五馬拉松概念了無新意、emoji 反應 2026-07-09 決定取消），週聯賽分組（30 人小組升降級）確認是之後想做的「更完整聯賽系統」，取代前兩者。
