...
>
> **🟠 #5 待辦（提醒：下次有 Android Studio 環境時做）— 開啟 splash 美化**：舊機/PWA 開啟時 Android 12+ 系統 splash 顯示圖示過渡較生硬。可改 Android 專案 `themes.xml` 的 `windowSplashScreenBackground` = `#05080f`（深色）+ TWA `SPLASH_SCREEN_BACKGROUND_COLOR` + 放 logo drawable，讓開場是深色霓虹而非預設白底。**需改 `android/` 並重打包 AAB 上傳**（純前端 push 無效）。圖示本身系統強制無法消除，只能美化。S24 PWA「閃一下瀏覽器列」是 PWA 測試假象，正式 TWA 不受影響（manifest 已 `display:fullscreen` + 深色 background_color）。
