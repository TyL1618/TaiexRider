...
> ## 🆕 2026-07-10（週五）家裡電腦晚場：三項體驗優化已完成，待打包 vc23
>
...
> 優化，**程式碼全部完成、typecheck 過、web 預覽無 error，待打包 vc23 上封閉測試**：
>
> 1. ✅ **Android 實體返回鍵修好**：換殼後返回鍵沒反應的根因是 Capacitor 把實體返回鍵
>    委派給 `@capacitor/app` 的 `backButton` 事件，而這外掛沒裝。已裝 `@capacitor/app`，
>    `App.tsx` 在原生殼註冊 backButton listener → `window.history.back()`，交給既有的
>    popstate 邏輯（子頁→首頁、首頁→確認離開、確認→`CapApp.exitApp()`），跟 TWA 時代
>    行為一致。⚠️ 真機驗證重點：① 子頁按返回回首頁 ② 首頁按返回跳確認、再按一次才離開
>    ③ 遊戲中按返回跳確認不直接離開 ④ 預測性返回手勢動畫還在（manifest 旗標保留；
>    若真機發現返回鍵在 Android 13+ 還是沒反應，備案是拿掉
>    `android:enableOnBackInvokedCallback="true"` 退回 legacy onBackPressed）。
> 2. ✅ **廣告預載入優化**：舊版點下去才 `prepareRewardVideoAd()`（把網路延遲壓在點擊
>    當下）。`ads.ts` 新增 `preloadRewardedAd(kind)`，進車庫/排行榜/遊戲時就背景備好，
>    播完自動再備下一支；`requestNativeRewardedAd` 優先用備好的、沒有才 fallback 現場備。
>    呼叫點：Garage 進場備 coin、DailyChallenge 進場備 coin、GameCanvas 進場備 revive、
>    結算彈出備 coin。⚠️ 真機驗證：看廣告的等待時間應明顯變短（尤其連續看第二支）。
> 3. ✅ **動畫開屏 Splash（取代舊「icon 閃一下」廉價感）**：`index.html` 的 boot-splash
>    升級成一次性品牌動畫（favicon 折線自己畫出→琥珀輪子彈入→TAIEX RIDER + slogan
>    淡入→霓虹讀取條），`main.tsx` 改成強制最短顯示 2200ms（原本 React 一 paint 就淡出
>    ＝秒開沒質感），仍以 React 已 paint 為前提避免露空畫面。裝 `@capacitor/splash-screen`
>    做原生冷啟動層（純色 `#05080f`，`drawable/splash_solid.xml` + 改 `styles.xml` launch
>    theme 背景 + `capacitor.config.ts` 設 `androidSplashResourceName`），`main.tsx` 原生殼
>    JS 一跑就 `SplashScreen.hide()` 交棒給 boot-splash，全程同底色無縫接。⚠️ 真機驗證：
>    冷啟動不應閃白圖，開場動畫完整播完約 2.2s 才進首頁。
>
> **下一步**：`versionCode 23`/`versionName 1.23` 已設好。打包簽署版 AAB 前**務必先
> `npm run build` 再 `npx cap sync android`**（已做過一次，但每次打包前都要確認 dist
> 是最新的，見「Capacitor 換殼」踩雷）→ Android Studio Generate Signed Bundle → 上
> **封閉測試**軌道。
>
