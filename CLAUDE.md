...
### 🔖 交接（2026-06-16 深夜 v0.3.3，車頂碰地即死）
...
1. 車頂（前擾流/風鏡/油箱/座椅）碰地 → 幾乎即死？
2. 正常飛行、空中後空翻 → 不誤判？
3. 陡坡朝上時屁股不誤觸死亡？（crashZone 沒延伸到尾殼）
4. crashZone 點位需要微調？（在 `src/game/constants.ts` 的 `BIKE.crashZone`，5 個 {x,y} 可隨時改）
