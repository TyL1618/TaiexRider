...
- [ ] 完美落地 A+C（規格見下）→ 進行中/已完成看 git log 有無 v0.12.7
- [ ] 留存①：PB 突破提示 + streak 連續參賽
- [ ] 留存②：經典獎牌制（銅/銀/金 × 12 關，localStorage）
- [ ] 留存③：分享卡視覺化（Canvas 圖卡 + navigator.share files）
- [ ] 留存④：全服死亡熱點（匿名彙總 RPC——需使用者跑小份 SQL——+ 賽道標記）
- [ ] 隱藏統計頁（連點版本號 5 下 + admin RPC 綁 email `tyl161803@gmail.com`）
