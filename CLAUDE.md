...
2. **鑽石車款（P 系列，5 台）＋鑽石購買頁**：P1（赤紅暴走）/P2（銀河鍍鉻）已開放測試上線（v0.12.27，車庫「鑽石車款」區塊可購買/裝備，價格 300/380 鑽石為暫定佔位值），P3~P5 尚未生圖仍「敬請期待」。**鑽石目前完全沒有獲取管道**（無廣告/任務/完賽獎勵），只有開發者測試帳號會補滿——之後要做一個購買頁（花台幣分別買鑽石／金幣），且需接 Google Play Billing（IAP）才能真正販售給一般玩家，鑽石車款真實售價/是否保留鑽石中介層屆時再決定。
3. **RETENTION_PLAN 第二批**（經典模式 Top N/百分位、經典週榜、週任務、股票圖鑑、狂暴盤日事件）：經典 Top N/週榜、**週任務、股票圖鑑**（2026-07-06 更新：這兩項若只存 localStorage 會有換帳號/裝置歸零的問題，跟暱稱/成就/streak 同一類，改判定為需要小量 schema）都**等使用者點頭 schema** 才能動工；只有狂暴盤日事件維持零 schema 可直接動工（每天重新判斷、無跨裝置狀態，獎勵倍率定案為金幣加成而非分數加成，避免碰到計分公平性）。
