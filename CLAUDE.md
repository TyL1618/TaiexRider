...
### 🔖 交接（2026-06-17 v0.5.1 — Phase 3 三模式 UI + Phase 4 prep）

**開工第一件事：`git pull`。**

> **Phase 3 完成（依使用者規格）**：首頁重構成三大入口。`App.tsx` 改為簡單路由（`home/custom/random/daily` + `track` 進遊戲，退出回原畫面）。各畫面自管 popstate（裝置返回鍵）：首頁→確認離開 App、子畫面→回首頁、遊戲→確認離開賽道。
> - **首頁** `src/screens/Home.tsx`：三大鈕（🏆每日排名賽[琥珀]／🎲隨機賽道／📈自選賽道）＋⚙設定（音量待實作＋版本＋更新日誌）。
> - **每日排名賽** `src/screens/DailyChallenge.tsx`：`data/pick.ts` 的 `dailyTrack()` 用**日期種子**選今日月盤地圖（全台同一天同圖、免後端）→ 走勢圖預覽＋名稱代號＋挑戰鈕。**排行榜佔位**（前 100 名・分數→時間排序）等 **Phase 4 後端**。
> - **隨機賽道** `src/screens/RandomSlot.tsx`：垂直拉霸機（`ITEM_H=72`、`VISIBLE=7`、`REPEAT=8`），rAF 動畫 2s 等速+2s 減速（速度 `v=D/(T1+T2/2)`，現 T1=T2=2），停 1s 浮現結果視窗（`Sparkline`＋名稱代號）。已用 preview 實測 landing index = 結果 stock 一致。
> - 共用：`src/components/Sparkline.tsx`（SVG 迷你走勢圖，末值vs首值上色）；共用 modal/back-btn/corner-btn 樣式移到 `index.css`。
> - **已用 preview 截圖驗證四畫面 layout 正常**（選單是純 DOM，不受隱藏分頁 rAF 暫停影響；拉霸 spin 的逐幀動畫只在可見分頁順跑）。

> **v0.5.1 微調**：manifest `display:fullscreen`（TWA 沉浸式）＋全面 `env(safe-area-inset-*)` padding（Android 三鍵列/瀏海不吃 UI）＋拉霸等速 3s→2s。
> **🟠 下一步 = Phase 4 後端（排行榜）**：完整設計見 **DEVDOC §11**。**prep 已 commit 完成（inert until `.env`，現有行為零改變）**：
> - `supabase/schema.sql`（daily_scores 表 + `submit_daily_score` upsert-if-better RPC + RLS + keep_alive）→ 直接貼 Supabase SQL Editor 跑
> - `src/lib/leaderboard.ts`（REST，`fetchDailyTop`/`submitDailyScore`，未設定 env 回 []/false）、`src/lib/playerId.ts`（匿名 UUID+暱稱）、`.env.example`、`data/pick.ts` `dailyKey()`
> - `DailyChallenge` 已串 `fetchDailyTop`（有成績列出、否則走 🔒 佔位）
> **明日只剩 DEVDOC §11.5 的 6 步**：開 Supabase 專案(新 org)→貼 schema.sql→填 `.env`→接 `GameCanvas onGameOver` 提交呼叫(+暱稱輸入,唯一要寫的 code 約十幾行)→cron-job.org 保活→真機測。身分採匿名裝置 ID+暱稱 MVP（Google 登入/防作弊後期）。每日地圖目前 client 端日期種子（嚴格同步+防作弊之後再用後端權威下發）。
