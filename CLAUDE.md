...
- **驅動（坡面切線鎖速）⭐ 核心模型**：著地按住 → 取「後輪→前輪連線方向（坡面切線，tx 永遠 > 0 = 恆朝前）」的速度分量，ease 到 `cruiseSpeed=7.2`。**任何坡角同速**（修「陡坡蝸牛」：舊水平鎖 vx=7.2，陡坡實際坡面速 = 7.2×cos(坡角) 超慢）；凸坡頂離地時有向上動量 → 飛更遠；tx 永遠正值 → 不倒退。
...
- **空中操控**：按住＝後空翻（`airSpinMax=0.192`、`airSpinAccel=0.024`，×0.8 of 原始值）；放開＝車頭極緩往前壓（`airNoseForwardAccel=0.0006`、`airNoseForwardMax=0.008`）。
...
1. 陡坡速度正常了嗎？任何坡角都能保持速度飛出去？（坡面切線鎖速）
2. 後空翻速度（×0.6）：好控了嗎？飛出瞬間不再後甩？
3. 結算畫面：HUD 消失、股票名稱顯示、按鈕不壓 ctrl-hint、版面乾淨？
4. 完賽後縮放全覽：車子停在終點不掉下去？
5. 翻滾後平行落地完美落地觸發？
6. 貼圖對位準不準？
