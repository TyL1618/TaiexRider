...
1. **反作弊機制（唯一真正「尚未動工」的大項）**：設計已在 [ANTICHEAT_DESIGN.md](ANTICHEAT_DESIGN.md) 定案多時，使用者 2026-07-03 明確拍板要做、**分派給 Fable 5 負責**、尚未寫一行 code。三階段：A（RPC 物理一致性驗證＋冷卻，隨時可上）／B（DB 端次數上限＋離群偵測，正式上架後）／C（操作事件序列＋Ghost，跟留存規劃的 Ghost 回放一起做）。**2026-07-04 下午：使用者安排 Fable 5 這時段主責反作弊＋整個遊戲資安檢查，Sonnet 同時段避開任何跟分數/排行榜/物理計分相關的改動，避免兩邊互相干擾。**
