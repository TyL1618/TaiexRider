...
- **✅ 車款改名+重新排序定價**：`garage.ts` 的 `BIKE_SKINS` P 系列（陣列宣告順序＝ Garage.tsx
  ...
  **⚠️ 使用者待辦：Supabase SQL Editor 跑 `migration_20260707c.sql`**（跑之前這些改動只有
  訪客/前端樂觀顯示生效，已登入玩家的真實扣款/發幣還是照舊金額）。
...
1b. **🐛 登出/切帳號不刷新 bug** ✅ **已修復**（2026-07-06）：不只是 `signOut()` 沒清錢包快取，
    ...
    `Garage.tsx`/`App.tsx` 客戶端配合。**⚠️ 待使用者跑 migration_20260706.sql + 真機用兩個帳號
    交叉驗證暱稱/成就不再互相污染**（preview 測不到已登入路徑）。
2. **鑽石車款（P 系列，5 台）＋鑽石購買頁**：✅ **2026-07-07 五台全數生圖完成上線**——P1（赤紅暴走）/P2（銀河鍍鉻）/P3（黃金大亨）/P4（電馭武士）/P5（幽靈匿蹤），車庫「鑽石車款」區塊皆可購買/裝備，價格 300/380/450/520/600 鑽石為暫定佔位值（IAP 真實定價待穩定後再決定）。**⚠️ 使用者待辦：Supabase SQL Editor 跑 `supabase/migration_20260707b.sql`**（`wallet_spend_skin()` 白名單補上 P3~P5，跑之前這三台購買會失敗）。鑽石真錢購買頁（`diamonds_100/350/1200` IAP）已於 2026-07-06 上線，但截至 2026-07-07 早上「暫無法購買」——判斷是 Play Console 商品剛建立+商家帳戶剛核准的正常同步延遲（首次設定常見 24~48 小時），已確認商品狀態「有效」、測試人員 opt-in 正常，非設定錯誤，持續觀察中。
...
9. **📌 正式上架後：清空伺服器所有玩家「玩過的遊戲數據」**（2026-07-04 使用者交代，尚未執行）——每日排名賽歷史成績（daily_scores/daily_scores_ranked）、經典模式紀錄（classic_records）、events 統計事件等全部歸零，讓正式版路人拿到乾淨新遊戲；**但已註冊的 Google 帳號/user_profiles 不能動**，只清「玩過的記錄」不清帳號。動手前務必先跟使用者逐表確認要清哪些 schema，不要自行判斷範圍。
