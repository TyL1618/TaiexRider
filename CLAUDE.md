...
### 🔖 交接（2026-06-16，chassis 圓形 + 死亡判定 + 完美落地修正 + 首頁更新日誌）
...
- **死亡判定**：`(翻倒>120° OR 雙輪離地) && 速度<5.0 && 持續 1s` → 判死。速度門檻 5.0 可避開正常飛行（cruiseSpeed=6.9>5.0），chassis 翻倒時接觸地面也能正確觸發（舊條件 0.5 被接觸力抖動排除）。
- `public/bike.png` 已就位（610×409 去背），貼圖生效。

**待使用者試玩確認（taiexrider.pages.dev）：**
1. 不再從折線接縫穿落掉出螢幕？（chassis 碰撞修正）
2. 重力 0.5 + 速度 6.912 手感？（上次 0.3 + 5.76）
3. 完美落地在陡坡/平路都能正確觸發？
4. 翻倒 1 秒可以正確判死？不再自動翻正？
5. 陡坡快到頂時不再抖動？（chassis 改圓形）
6. 首頁「ℹ v0.3.0」按鈕能看到更新日誌？
7. 結算時 HUD/設定鈕/返回鈕 不再重疊？
