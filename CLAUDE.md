...
### 🔖 交接（2026-06-18 — TWA 全螢幕 + 返回鍵修正 + v0.9.3 功能補強）

**開工第一件事：`git pull`。**

> **本次完成（TWA 完整修正 + v0.9.3）**：
>
> **TWA 核心問題修正**
> - **assetlinks.json**：Google Play App Signing 重新簽署後原 keystore SHA-256 失效，已更新為 Google Play 簽署金鑰（`DB:F0:8B:8F:...:57:DA`）→ TWA 驗證成功，App 全螢幕無網址列。
> - **TWA 全螢幕 immersive**：
>   - `android/app/src/main/res/values/themes.xml` 與 `values-night/themes.xml`：parent 從 `DarkActionBar` 改 `NoActionBar` + 加 `windowFullscreen`、透明 status/nav bar、`shortEdges` cutout。`DarkActionBar` 會與 immersive mode 衝突。
>   - `android/app/src/main/AndroidManifest.xml`：LauncherActivity 加 `DISPLAY_MODE=immersive-sticky` meta-data，androidbrowserhelper 才會啟用 sticky immersive（底部導覽列收起，邊緣滑動暫時顯示）。
> - **返回鍵 race condition**：`App.tsx` 的 `confirmLeaveRef.current` 改在 `setConfirmLeave` 前**同步**更新，避免快速連按時第二個 popstate 讀到 React render 前的舊值。
> - **確認離開無效**：`doLeave` 的 `window.close()` 在 TWA 被 Chrome 封鎖，改加 `history.go(-(history.length+5))` 耗盡 history 讓 Android Activity 自然 finish。
>
> **v0.9.3 功能**
> - **音量控制**：settings modal 加滑桿；`audio.ts` 加 master gain node，音量存 localStorage `taiexVolume`（預設 0.8）。
> - **首頁文案**：三按鈕描述更新；移除自動更新測試標記。
> - **隱私權政策**：`public/privacy.html` → `taiexrider.pages.dev/privacy`（Play Store 必要）。
> - **ManageDataLauncherActivity 修正**：manifest 補宣告，修正 2.7.1 啟動閃退。
>
> **⚠️ 待真機驗證（需重新打包 AAB 上傳）**：
> - 全螢幕 immersive-sticky（底部導覽列是否收起）
> - 快速連按返回 → 應跳確認而非直接退出
> - 確認離開 → App 應確實關閉
>
> **Android Studio 同步提醒**：改 `android/` 後要手動複製到 `C:\Users\tyl16\AndroidStudioProjects\TaiexRider\` 對應路徑，`versionCode +1`，重新 Generate Signed Bundle 再上傳。
>
> **Google Play 現況**：
> - 帳號：Harold_Yun（tyl161803@gmail.com），已過身分驗證
> - App：TAIEX RIDER（com.tylapp.taiexrider），內部測試軌道
> - Keystore：`C:\Users\tyl16\Documents\taiexrider-release.jks`（alias: taiexrider）⚠️ 僅在公司電腦
