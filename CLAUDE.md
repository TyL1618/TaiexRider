...
### 🚧 交接給新 SESSION（2026-06-15，已討論定案、尚未動工）

**目前線上版本 commit = `7e6d962`**（沿坡恆速 + gravity 1.5）。以下 4 項**已和使用者討論定案、等動工**。使用者開新 session 接手，請先 `git pull`，動工前再向使用者確認一句即可。

**已確認的根因**：使用者最後回報「**車尾貼地、車頭翹起時，車頭下不來（卡在翹起太久）**」——這是**著地時發生、與空中無關**。元凶＝`GameCanvas.tsx` `applyControls` 裡我加的「**著地角速度阻尼**」(`av = angularVelocity*0.5` 再夾 `±0.12`)：它把「重力想把車頭轉下來」的角速度也一起壓死了，所以車頭轉不下來。空中墜落速度 OK 甚至太重。

**待做（依定案）：**
1. **拿掉著地角速度阻尼** → 讓重力自然把車頭壓下來。
   - 位置：`src/game/GameCanvas.tsx` `applyControls`，grounded 分支裡那段 `let av = c.angularVelocity * 0.5; ... AV_MAX=0.12 ... setAngularVelocity`。
   - 做法：整段移除；**若**移除後開場/撞陡坡又亂轉，改成「只夾極端轉速」溫和版（如 `|av|>0.35` 才夾），**不可**再壓正常旋轉。
2. **重力調回 1.0**（現在 1.5 太重）：`GameCanvas.tsx` `engine.gravity.y = 1.5` → `1`。
   - 連帶把後翻參數調回原值：`constants.ts` `airSpinMax 0.18→0.12`、`airSpinAccel 0.009→0.006`。
   - `cruiseSpeed = 9` **保留**（使用者說速度感很好）。
3. **車身縮小到 ~52%**（接近 Rider 比例，順帶減少翹翹板/甩晃/過山頂彈飛）：`constants.ts` `BIKE`：
     chassisW 92→~48、chassisH 20→~10、wheelRadius 11→~6、wheelBaseHalf 34→~18、wheelDropY 13→~7。
   - ⚠️ 改完檢查 `GameCanvas.tsx` 的 `spawnY = vertices[0].y - 55`（生成高度）是否要跟著縮、以及車體繪製是否正常。
4. **加距離分**（特技分之外再加行進分）：
   - 公式：行進分 = `(c.position.x / track.finishX) * 1000`（到終點剛好 1000），即時更新。
   - 特技分（後翻/完美落地）**疊加在行進分之上**。HUD 顯示總分 = 行進分 + 特技分。
   - 位置：`GameCanvas.tsx` `step()` 內 `points`，及 HUD `setHud`。

**使用者明確否決**：空中「自動擺平」不要加（要保留 Rider 那種不翻好就摔的技術性）。

**動工後照守則**：改 code → 更新 DEVDOC/CLAUDE.md + commit + push（GitHub Actions 自動部署）。
