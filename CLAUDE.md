...
> fetcher）。`npm run build`＋`npx cap sync android` 已跑。**versionCode 維持 32**
> （07-15 批次推進的 vc32 尚未打包，本批直接併入；若 vc32 其實已上傳過，打包前
> 記得再 +1）。PWA push 即自動生效——修復當天線上玩家就能玩到正確的 7/17 地圖。
>
