...
- **驅動（兩輪取坡 + 保留垂直分量）**：著地按住 → 沿「前後輪所踩地形連線(弦) `chordSlope`」方向鎖速 `cruiseSpeed=7.2`，**只改切線分量、保留法線(垂直)分量**。用兩輪取坡而非車身中心 → 前輪上陡坡整台車跟著轉上去（不用鼻頭水平爬）；保留垂直分量 → 過坡頂自然彈飛、不黏地。
- **速度解耦**：地面 `cruiseSpeed=7.2`（前版 12 ×0.6，慢好控）；**離地瞬間速度 ×`launchBoost=1.67`** → 飛行距離仍≈前版 12。「慢動作但飛一樣遠」。
- **離地歸零殘留角速度**：消除爬坡貼坡帶上來的「莫名往後翻」。
- **空中操控**：按住＝後空翻（負向）；**放開＝車頭緩緩往前壓**（`airNoseForwardAccel=0.006`、`airNoseForwardMax=0.08`，緩、有上限不自己翻整圈）。⚠️ 這推翻了先前「空中不要任何自動旋轉」的否決——**使用者本次明確要求**放開時自動低頭備降。
- **前壓配重**：前輪 `frontWheelDensity=0.0030` > 後輪 `0.0012`（地面防翹頭仍保留）。
- **落地貼地**：著地角速度朝弦坡比例修正（`groundAlignGain=0.3`，夾 `groundedAvMax=0.28`）；`restitution=0.05`。
...
1. 車速 7.2 好控嗎？飛行距離有保持（launchBoost）？
2. 陡坡 / V 谷還會卡死或鼻頭水平爬嗎？（兩輪取坡）
3. 過坡頂飛得出去了嗎？（保留垂直分量）
4. 空中放開→車頭往前壓的感覺對嗎？按住→後空翻正常？
5. 翻滾後平行落地，完美落地會觸發了嗎？
6. 貼圖對位準不準？
