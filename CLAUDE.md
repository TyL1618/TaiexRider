...
>
> **✅ 已修（使用者拍板：現在動工改程式碼+做圖示資源，但不打包上傳，等正式版審核
> 過再一起包）**：從 `public/icon-512.png`（走勢線+雙輪造型）萃取前景線條，去背轉成
> 純白剪影，產出 `android/app/src/main/res/drawable-{m,h,xh,xxh,xxxh}dpi/
> ic_stat_notify.png` 五組密度；`notifications.ts` 加上 `smallIcon: "ic_stat_notify"`
> + `iconColor: "#ffb300"`（品牌琥珀色，對應 `--neon-amber`）。
>
> typecheck/build 過、`npx cap sync android` 已跑（9 個外掛都在）。**這是原生殼專屬
> 改動，PWA 網頁版本來就沒有通知功能不受影響**；`git push` 只會觸發網頁版部署，不影響
> Android。**這次刻意不打包簽署版 AAB、不上傳 Play Console**——versionCode 維持
> **33**（跟 7/18 那批「移除靜態賽道樣本」bug 修復併在同一顆待打包版本），等正式版
> 審核結果出來後再一起處理打包上傳。真機驗證（狀態列圖示是否正確顯示）要等那次打包
> 裝上手機才能確認。
...
> ——修復當天線上玩家就能玩到正確的 7/17 地圖；**Android 端要等 vc33 打包上傳**，
> 在那之前 vc32 以前的所有版本選熱門股仍會玩到內建的 6/15 舊盤。
>
