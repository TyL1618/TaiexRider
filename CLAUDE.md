...
> - **#2 「確定離開」關不掉 App**：`App.tsx` `doLeave` 從 `history.go(-1)` 改 `window.close()`（TWA/PWA 才能真正關閉；go(-1) 只退回初始頁仍在 App 內）。參考 UFO_Duel 作法。
> - **#3 子頁連按兩次返回直接關遊戲（race）**：根因＝單一 listener + 純 state 切頁、無真實 history 深度，原生返回快過 popstate 內補推哨兵。修法：`handleNav` 進子頁時 `pushState` 一層真實 entry；`goHome`（子頁返回鈕）改 `history.back()` 由 popstate 統一切首頁；popstate handler 只在「首頁→離開」邊界補推哨兵，「子頁→首頁」不補推（已停在首頁哨兵）。
