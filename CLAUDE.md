...
- **空中操控**：按住＝後空翻（`airSpinMax=0.144`、`airSpinAccel=0.018`，已×0.6 降速，同時降飛出瞬間後甩力）；放開＝車頭極緩往前壓（`airNoseForwardAccel=0.0006`、`airNoseForwardMax=0.008`）。⚠️ 放開時自動低頭備降（使用者本次明確要求）。
...
- **結算畫面**：`.overlay-result`（透明不變暗不模糊）讓出中段看股市折線圖。進結算 → HUD（分數/計時器/油門燈/操作提示）全隱藏，避免與 overlay 重疊。結算顯示：標題 + 股票代號名稱 + 分數 + 時間 + 按鈕。按鈕往上移、標題往下移（`padding: 18% 0 16%`）。
- **完賽停車**：完賽瞬間 `Body.setStatic` 凍住車身，避免縮放全覽時車子掉出賽道邊界；`doReset()` 時解凍。
