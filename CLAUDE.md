...
> ## 🎰 2026-07-21：抽獎轉輪 + 鑽石新出口——規格與 SQL 已備妥，**前端尚未動工**
>
> 使用者討論後拍板一個新方向：鑽石目前唯一出口是 P 系列車款，對車無興趣的玩家
> 鑽石沒有消費動機。動工前先討論定案完整規格（機率表/黑天鵝隱藏車款/票券/
> 尾焰特效顏色/鬼影顏色/暱稱顏色/稱號/前綴圖示），使用者要求先整理成文件+SQL
> 避免遺漏，**這批只做規格書+後端 SQL，前端 UI/程式碼下一步才開始動工**。
>
> **✅ 已完成**：
> - [LOTTERY_DESIGN.md](LOTTERY_DESIGN.md)：完整規格書（機率表定案版含 EV 校準、
>   黑天鵝重複保護、鑽石新出口清單與提案售價、車庫 UI 調整、技術要求）。
> - `supabase/migration_20260721.sql`（⚠️ 待使用者手動跑，SQL Editor 執行前不影響
>   任何現有功能）：`player_wallet` 新增 `tickets` 欄位；新表
>   `wallet_daily_lottery`（每日 2 次免費抽獎計數，比照 `wallet_daily_attempts`
>   模式）；`wallet_get()` 改連票券一起回傳（改了輸出欄位，用 drop+recreate，
>   前端 `garage.ts` 呼叫端等前端動工時要跟著補讀 `tickets`）；新 RPC
>   `lottery_spin(p_paid)`（機率表寫死在函式裡，重複保護自動換算鑽石補償，黑
>   天鵝額外贈稱號+徽章）、`lottery_state()`、`wallet_spend_item()`（新道具
>   購買，價格白名單）、`consume_ticket()`、`wallet_earn_via_ticket()`。
> - **本批 SQL 全程刻意避開 `set coins = coins + x` 這種容易撞名的寫法**（改成
>   先讀進 `v_coins` 局部變數算好再整段賦值），比照本專案已踩過兩次的 42702
>   地雷教訓，寫的時候就避開，不是憑經驗僥倖。
>
> **⏳ 尚待確認的細節（見 LOTTERY_DESIGN.md §9）**：票券實際賺取來源、暱稱顏色/
> 稱號/前綴圖示的完整內容與售價（目前是提案）、黑天鵝重複補償 800 鑽是否合理、
> 尾焰/鬼影顏色實際色票。**黑天鵝美術風格已定案**：使用者選「黑天鵝」（台股術語，
> 呼應 0.05% 極端稀有機率），黑金神秘配色，之後交給 Grok 出圖。
>
> **另外順手排進待辦（非本次規格範圍，跟抽獎功能無關）**：`RandomSlot.tsx`
> （隨機選賽道拉霸機）整體要往上移、縮短 1~2 個項目高度，讓底部按鈕更好按——
> 動工時一起做。
>
> **下一步**：使用者確認 §9 待確認細節後說「動工」，才開始寫前端 UI（抽獎拉霸機
> 新畫面、車庫三分類+票券貨幣列、廣告前彈窗詢問是否消耗票券）+ 跑 SQL migration。
