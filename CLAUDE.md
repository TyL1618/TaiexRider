...
### 🔖 交接（2026-06-16 深夜，翻正 bug + 地形穿落根治）
...
- **死亡判定（⭐最新邏輯）**：① `翻倒>120° && 有地面接觸` → 立即進 1 秒倒計時（同時鎖轉，見下）；② `雙輪離地 && 速度<0.5` → 空中卡死。正常飛行中速度 ≈ 6.9，空翻中沒有地面接觸，兩條件均不觸發。
- **翻倒旋轉鎖定（⭐新）**：每 step 若 `upsideDown && hasGroundContact`，呼叫 `Body.setAngularVelocity(chassis, 0)`，徹底阻止前輪配重（frontWheelDensity=0.003 > rear=0.0012）的「不倒翁效應」，車翻後不會自動翻正。
- `public/bike.png` 已就位（610×409 去背），貼圖生效。

**待使用者試玩確認（taiexrider.pages.dev）：**
1. 翻車後不再自動翻正？（旋轉鎖定）
2. 翻倒貼地 1 秒後確實死亡？（無速度門檻，只要翻倒接觸地面即進倒計時）
3. 地形接縫穿落已解決？（請讓朋友家人確認，截圖那個 bug）
4. 正常飛行 / 空中後空翻不誤判死亡？
5. 首頁「ℹ v0.3.2」能看到更新日誌？
