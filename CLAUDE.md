...
- **驅動（前輪領坡 + 保留垂直分量）**：著地按住 → 沿「**前輪所在坡段** `slopeAt(前輪x)`」方向鎖速 `cruiseSpeed=7.2`，**只改切線分量、保留法線(垂直)分量**。用前輪領坡（不是弦/車身中心）→ 前輪一貼上坡牆，驅動沿牆面往上＝**銳角 V 谷(<90°)/陡牆都爬得上去**；保留垂直分量 → 過坡頂自然彈飛、不黏地。
- **速度解耦（已修爆衝）**：地面 `cruiseSpeed=7.2`；離地**拉到目標速 `cruiseSpeed×launchBoost≈12`、但設上限永不超過**（不再用乘法→不疊乘爆衝），且要**在地面連續待夠 `minGroundedStepsForBoost=5` step**才給（擋轉折點微彈觸發）。飛行距離仍≈前版 12。
