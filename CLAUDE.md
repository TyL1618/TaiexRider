...
- **低重力**：`engine.gravity.y = 0.5`（原 1.0）→ 空中時間 ×2，翻轉窗口更寬（Ketch Rider 風格）。
...
- **空中操控**：按住＝後空翻（`airSpinMax=0.192`、`airSpinAccel=0.024`）；放開＝車頭極緩往前壓（`airNoseForwardAccel=0.0006`、`airNoseForwardMax=0.008`）。
...
1. 上坡不再彈跳？行進速度穩定不一陣一陣？（法線歸零 + groundLockEase 0.7）
2. 陡坡能飛高了嗎？（彈跳消除 → groundedStreak 正常 → boost 觸發）
3. 空中時間夠長、翻轉容易控制？（gravity=0.5）
4. 落地仍順暢不彈飛？（法線歸零只拿掉離坡速度，不干涉落地衝擊）
