...
### 🔖 交接（2026-06-17 v0.5.0 — Phase 3 三模式 UI）
...
> - **隨機賽道** `src/screens/RandomSlot.tsx`：垂直拉霸機（`ITEM_H=72`、`VISIBLE=7`、`REPEAT=8`），rAF 動畫 3s 等速+2s 減速（off 公式 `v=D/4`），停 1s 浮現結果視窗（`Sparkline`＋名稱代號）。已用 preview 實測 landing index = 結果 stock 一致。
...
> **🟠 下一步 = Phase 4 後端（排行榜）**：設計已寫進 **DEVDOC §11**（schema/RLS/RPC/匿名身分/提交流程/防作弊路徑 + 明日執行清單）。每日地圖目前是 client 端日期種子（可動，嚴格同步＋防作弊之後再用後端權威下發）。
