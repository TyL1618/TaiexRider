...
### 🔖 交接（2026-06-23 — 懸空開始機制 + 廣告偵測骨架（第一階段））

**開工第一件事：`git pull`。**

> **#1 懸空開始（公平計時）**：任何模式進場/復活，車子生成於地面上方 100px 並凍住（static body），顯示「觸碰螢幕開始 / TAP TO RIDE」閃爍提示。首次觸碰才解凍、計時器才跑、引擎聲才啟動。空中後翻鎖定到首次落地（`hasEverGrounded`），避免懸空落下時誤觸翻滾。`doReset()` 復活同樣回懸空等待。復活落下時間＝自然懲罰，秒數從動了才接續＝公平。
>   - **修過的 bug**：開場立刻爆炸——懸空等待時車身 static、velocity=0、雙輪離地，`stuckMidAir`(bothWheelsOff && speed<0.5) 成立 → crashTimer 0.1s 後判死。修法：`tippedOver`/`stuckMidAir` 兩個死亡條件加 `!waitingToStart` 守衛。
>   - 檔案：`GameCanvas.tsx`（spawnY 抬高、3 個 body setStatic 凍/解、`waitingToStart`/`hasEverGrounded` 旗標、計時 `&& !waitingToStart`、死亡守衛）；`GameCanvas.css`（`.start-prompt` + `@keyframes startPulse`）。**未真機試玩。**
>
> **#2 廣告雙軌偵測骨架（第一階段，⚠️ 目前不顯示任何廣告）**：
>   - 新增 `src/lib/ads.ts`：`isInsideTWA()`、`detectEnv()`、`initAds()`（只有「非 App」且 `ADSENSE_PUB_ID` 非空才載 AdSense）。
>   - **⚠️ 偵測訊號（真機實測定案，別再改回 referrer）**：本專案 TWA **不送** `android-app://` referrer（實測 `document.referrer` 是網站自己網址）→ referrer 偵測無效。**改用 display-mode**：TWA（themes.xml + manifest 皆 `fullscreen`）回報 `fs`、瀏覽器 `browser`。規則＝**「Android 且 display-mode 非 browser」→ 視為 App 不載 AdSense**。Android 瀏覽器網頁(`browser`)照載、iOS 一律照載（非 Android）。**不快取**（TWA Custom Tabs 與 Chrome 同機共用 localStorage 會誤判；display-mode 每次即時讀）。真機實測：手機 TWA→App、桌機/手機瀏覽器→網頁，分流正確。
>   - **`ADSENSE_PUB_ID = ""`（留空）= 第一階段任何環境都不載入廣告**。`main.tsx` 呼叫 `initAds()`。`Home.tsx` 設定面板版本列加「・App／・網頁」顯示 `detectEnv()` 供日後快速複驗（驗證用詳細診斷行已移除）。
>   - **不影響 Google Play 審查**：TWA 不載廣告、無 pub ID 也不顯示，維持商店「廣告：無」宣告。
>   - **第二階段（AdSense 審過 + 真機驗證偵測 OK 後）**：填入真實 `ADSENSE_PUB_ID`（`ca-pub-8981745966447649`）+ 廣告版位，網頁版才開始顯示。Android AdMob Rewarded 另在原生層做。詳見下方「廣告雙軌架構」。
>   - **AdSense 帳號**：付款帳戶已建（pub-8981745966447649），收款銀行待有收益後系統才開放填。
>   - typecheck + build 通過。
...
- **廣告雙軌架構（2026-06-23 決策，正式上架後實作）**：
  ...
  - **避免雙重廣告**：TWA 啟動時 `document.referrer = "android-app://com.tylapp.taiexrider"`，用以下邏輯分流：
    ```javascript
    const isInsideTWA = document.referrer.startsWith('android-app://');
    if (!isInsideTWA) {
      loadAdSense(); // 只有網頁版才載入 AdSense
    }
    // AdMob 由 Android 原生層呼叫，不在網頁層處理
    ```
