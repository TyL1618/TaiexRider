...
> ## 🐛 2026-07-10 深夜：抓到「騎乘中車子陷進地形」根因 + 做好 DEV 調參面板（尚未決定是否改預設值）
>
...
>
> **已完成（`PHYSICS.subSteps` 預設仍是 1 ＝ 完全不動現行手感）**：
> - `PHYSICS` 子步支援（`constants.ts` / `GameCanvas.tsx`）。headless 驗證：`subSteps=2`
>   深陷率 15.3%→0.3%、`=3` 歸零；完賽平均步數 2729 vs 2755（**速度等價，換算正確**）。
> - 🔴 **重要副作用**：`subSteps=1` 那 800 局裡有 **119 次 `topHit` 死亡，修好後全部消失**
>   ——代表現行版本**約 15% 的死亡其實是這個 bug 造成的**，不是玩家技術問題。修掉會讓
>   遊戲明顯變寬容，這是預期中的行為改變，要讓使用者知道。
> - **DEV 調參面板**（見下方踩雷筆記「🎛️ DEV 調參面板」整節）：滑桿即時生效、即時 sink
>   讀數、面板內建「跑 N 局自動驗證」回報沉沒率、可匯出貼回 `constants.ts`。
>   `vite.config.ts` 已開 `server.host` 讓手機連區網實機調手感。正式 build 已驗證不含面板。
>
> **⬜ 下一步（等使用者決定）**：用面板在**真機**上把 `subSteps` 切到 2/3 感受手感與 FPS，
> 決定要不要把 `constants.ts` 的 `PHYSICS.subSteps` 預設改掉。改了才算真的修好 bug。
> 也可比較「加大 `wheelRadius`」這條替代路線（面板同一組滑桿可直接試）。
>
