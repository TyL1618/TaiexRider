...
- **驅動（水平鎖朝前、垂直交給物理）⭐ 核心模型**：著地按住 → **水平速度 ease 到 `+cruiseSpeed=7.2`（永遠朝前，賽道恆 +x 為前進），垂直分量 (y) 不碰**。水平鎖朝前 → 絕不倒退（修「按了往後跑」：之前「維持速度大小不動方向」會把重力造成的後滑速度放大→倒退）；垂直自由 → 上坡靠地面頂著爬、凸坡頂保留往上動量**自然飛出去**(往右上就飛)、凹谷貼地不亂飛。起步/後滑一按即轉正，不需 kick。
...
- **結算畫面**：改用 `.overlay-result`（透明不變暗不模糊、標題置頂按鈕置底）讓出中段看股市折線圖；設定 modal 仍用 `.overlay`（置中變暗）。
