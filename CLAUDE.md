...
### 🔖 交接（2026-06-16 凌晨 v0.3.4，死亡特效 A+B）
...
**死亡特效（⭐ v0.3.4 新增）：**
- 翻車觸發後 0.1s：車身位置爆出 28 顆粒子（琥珀×2/青/白隨機），速度 3-12px/step，隨重力 0.35 散落
- 同時：1 幀白色全屏閃光（`deathFlashAlpha=1.0`，每幀 ×0.72 淡出）
- 鏡頭震動：`deathShakeAmp=8px`，每幀 ×0.82 衰減（暫時偏移 camX/camY，渲染後還原不汙染平滑狀態）
- 動畫期間 `dying=true`：HUD/分數/按鈕全隱藏；0.8s 後 `crashed=true` → 進結算畫面

**待使用者試玩確認（taiexrider.pages.dev）：**
1. 翻車後粒子爆炸 + 白閃 + 震動效果滿意嗎？
2. 車頂碰地死亡判定精準度？（調 `BIKE.crashZone` 5 點座標）
3. 粒子參數想調整？（大小/數量/顏色/重力 in `spawnDeathParticles` in `GameCanvas.tsx`）
...
0. **先試玩** `taiexrider.pages.dev`，確認死亡特效＋車頂碰地判定
