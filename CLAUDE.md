...
>   - 新增 `src/lib/ads.ts`：`isInsideTWA()`（`document.referrer` 起頭 `android-app://` → 是 TWA；**偵測到後寫 localStorage `tr_is_twa` 記住**，防 TWA 內重整 referrer 消失漏判害廣告漏進 App）、`detectEnv()`、`initAds()`（只有「非 TWA」且 `ADSENSE_PUB_ID` 非空才載 AdSense）。
>   - **`ADSENSE_PUB_ID = ""`（留空）= 第一階段任何環境都不載入廣告**。`main.tsx` 呼叫 `initAds()`。`Home.tsx` 設定面板版本列加「・App／・網頁」顯示 `detectEnv()` 供真機驗證偵測。
