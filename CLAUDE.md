...
### 💰 2026-07-08：金幣/鑽石經濟大改版（使用者拍板 6 項，Sonnet 一次動工完成）

使用者提出完整經濟設計討論後拍板，一次做完六批（每批各自小範圍，但因彼此高度相關這次
合併一次 commit）。**⚠️ 需要使用者手動在 Supabase SQL Editor 依序跑 4 份新 migration**：
...
鑽石結算）、`migration_20260708d.sql`（經典模式週結算+鑽石）。跑完前，新功能會 fallback
成本地樂觀值或靜默略過（不影響現有功能運作），不會讓 App 崩潰。
...
- **驗證狀態**：typecheck + 全站 build 皆過；preview 實測 Garage 定價顯示 500/500 正確、
  ...
  結算實際發鑽石**這三項因為需要真實資料庫資料(migration 跑完)或需要真機才能看到完整死亡
  動畫轉場（preview 隱藏分頁 rAF 暫停的已知限制），暫未能實測，等 migration 跑完+真機/
  桌機測試確認。
