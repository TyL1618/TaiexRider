...
**目前驅動 / 手感（定速引擎 + B 穩定性修正）：**
- **驅動**：著地按住 → 車身+雙輪速度**沿 `slopeAt()` 坡面切線方向**鎖成 `cruiseSpeed=12`（pull 版 15 的 80%）。沿坡面切線而非車身角度 → 不重演翹頭正回饋；直接設速度 → 任何坡都爬得上、上下坡平地同速。離地即停鎖＝拋體，靠離地速度+坡角自然飛出去（gravity 維持 1，不動）。
...
- **完美落地新規則**：必須**先在空中完成翻滾 `flips>0`** + 真實跳躍 + 雙輪同時觸地 + 正立才觸發；
  車身輕輕彈起雙輪一碰就觸發＝沒意思，已擋掉。
- 死亡判定不變：`|vx|<0.5 && (throttle||chassisContacts>0)` 持續 2.0s。
- `public/bike.png` 已就位（610×409 去背 PNG），貼圖生效；對位調 `BIKE.spriteW/spriteOffsetX/spriteOffsetY`。

**待使用者試玩確認（taiexrider.pages.dev）：**
1. 車速 `cruiseSpeed=12` 快慢合適嗎？（太慢調高、太快調低）
2. 緩上坡 / 陡上坡都爬得上了嗎？（定速直接設速度，理論上必上得去）
3. 能飛得出去、飛得夠高、空中有空檔翻兩圈嗎？
4. 落地穩穩貼地、不彈跳不翻車？
5. 平地還會不會翹頭後翻？
6. 貼圖對位準不準？
