...
- **家裡 AS 專案已換血**：公司帶回的最新版已覆蓋 `C:\Users\tyl16\AndroidStudioProjects\TaiexRider\`（舊 6/18 版備份在 `TaiexRider_old_0618`，確認 vc10 能 build 後可刪）。**目前家裡＝最新正統版本**。
- **流程教訓**：上傳前先 Generate Signed APK 給 Claude `adb install -r` 真機驗證能開機，再上傳 AAB——不再盲上。
...
**⚠️ 使用者待辦（回家前/回家後）**：
1. Supabase SQL Editor 跑 **`supabase/migration_20260702b.sql`**（死亡熱點 + 統計頁 RPC；第一份 migration_20260702.sql 已跑過）。
2. ~~AS 專案已就緒（versionCode 9）~~ **vc9 會秒崩（見上方事故記錄），已修為 vc10**：先 Generate Signed **APK** 給 Claude adb 驗證 → OK 再 Generate Signed **Bundle** 上傳封測軌道 → 真機測返回鍵/splash 圖/長按捷徑。
3. 真機測 v0.12.7~12：完美落地合併 toast、PB 徽章、streak、分享圖卡（面板帶圖）、死亡熱點（migration b 跑完+今晚有人玩後）、統計頁（開發者帳號登入）。

**🏠 家裡 Claude 剩餘任務**：
- [ ] **平地縫隙卡輪**（新回報 bug）：高速墜落、落點正好在兩段平坦梯形共用垂直邊 → 輪子楔入縫隙（放開可脫困）。做法：`scripts/simStuck.ts` 加「定點高空落下」測試模式（不同高度 × 接縫偏移 × 幾千次）量化，再測修法（候選：加大 `topExtra` 接縫重疊、落地法線處理），模擬驗證後才動 terrain.ts。參考本次 V 谷修法流程。
- [ ] 留存後續批次見 [RETENTION_PLAN.md](RETENTION_PLAN.md)（週任務/圖鑑/經典週榜需使用者點頭 schema）

**完美落地 A+C 定案規格**：圈數判定從 `floor(|rot|/2π)` 改「差 0.3π 內進位」＝`floor((|rot|+0.3π)/2π)`；翻轉分/完美分/totalFlips **全部用同一套圈數**；完美落地時合併為單一 toast「完美落地 N 圈 +{翻轉分+完美分}」。分數水位微升、全體一致。
