...
- **資料量不爆**：寫入用 upsert `Prefer: resolution=merge-duplicates`，衝突鍵 `(map_date, stock_code)`。連假多次跑都是「覆蓋同一批 ~1090 列」非新增；另每次清除 7 天前舊資料。
- **app 讀取（連假安全）**：`src/lib/dailyMap.ts` 的 `resolveSessionDate()` 取「daily_map 中 `map_date ≤ nextDay(今天)` 的**最大值**」當「目前這一期」，三個 fetcher（hardest/list/stock）全部對齊它。⚠️ **不可用「今天/明天」精準比對**：`map_date = sessionDate+1` 只覆蓋 session 後一天，連假**第二天**起日曆日就超過它 → 查無 → 掉回靜態 24 支（2026-06-20 週六發生過：週四盤存在 `map_date=6/19`，週六日曆日 6/20 的舊視窗 `[6/20,6/21]` 整個錯過）。用 `lte + max` 才能「一律顯示最後一次盤勢」。
- **排行榜對齊（同一張榜跨連假）**：challenge key 也用 `resolveSessionDate()`（= `max(map_date)`），讀取（`DailyChallenge`）、提交清快取（`leaderboard.ts`）、RPC 寫入（`submit_daily_score` 的 `v_today := coalesce(max(map_date)≤台灣今天+1, 台灣日曆日)`）三者同源 → 連假整段成績累積在同一張榜，不隨日曆日換到空榜。**schema 改完要手動在 Supabase SQL Editor 跑 `create or replace function submit_daily_score`，push 不會更新 RPC。**
...
> **本次修的 bug（連假第二天觸發）**：週四(6/18)是最後交易日，週五六日連假。週五正常顯示週四盤；**週六**卻掉回最原始的靜態 24 支測試盤、排行榜也跑掉。
...
> - **修法（一律顯示最後一次盤勢 + 連假同一張榜）**：
>   - 新增 `resolveSessionDate()`（`dailyMap.ts`）= daily_map 中 `map_date ≤ nextDay(今天)` 的 **max**。三個 fetcher 改用它精準比對（不再 `[today,nextDay]` 迴圈）。
>   - 排行榜讀取/重整（`DailyChallenge`）、submit 清快取（`leaderboard.ts`）、App 預熱（`App.tsx`）全部改用 session key。
>   - **⚠️ RPC 待手動執行**：`supabase/schema.sql` 的 `submit_daily_score` 改 `v_today := coalesce((select max(map_date) from daily_map where map_date ≤ 台灣今天+1), 台灣日曆日)`。**push 不會更新 RPC，要進 Supabase SQL Editor 跑 `create or replace function submit_daily_score`** 才生效，否則寫入端仍用舊日曆日 → 連假成績仍掛錯 key。
> - typecheck 通過。詳見上方「每日地圖資料管線」段的「app 讀取（連假安全）」「排行榜對齊」。
