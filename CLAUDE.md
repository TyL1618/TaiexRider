...
### 🔖 交接（2026-06-18 — Phase 7 TWA 打包進行中，遊戲功能全部正常）

**開工第一件事：`git pull`。**

> **今日進度（Phase 7 準備）**：
> - **PNG icon 新增**：`public/icon-192.png` + `public/icon-512.png`（由 `scripts/gen-icons.mjs` 從 favicon.svg 產生）。manifest 已加入，Play Store 必要條件。
> - **改名同步 SQL 已執行**：Supabase `user_profiles` table + `daily_scores_ranked` VIEW 均已建立，`leaderboard.ts` 已切換查 VIEW，改名同步功能**完整生效**。
> - **本機開發環境（家裡桌電）已安裝**：
>   - JDK 17：`C:\Program Files\Microsoft\jdk-17.0.19.10-hotspot`
>   - JDK 21：`C:\Program Files\Microsoft\jdk-21.0.11.10-hotspot`
>   - Android Studio 2026.1.1 Patch 1（已安裝，開啟正常）
>   - Bubblewrap：已安裝但 Node 24 相容性問題，**不要用 Bubblewrap**
>
> **🟠 明天繼續（公司電腦）**：
> 1. 在瀏覽器開 **https://pwabuilder.com**，輸入 `https://taiexrider.pages.dev`
> 2. Package For Stores → Android Package Options：
>    - Package ID：`com.tylapp.taiexrider`
>    - Include source code：**打勾**
>    - 選 **Other Android** tab（不走 Google Play 簽名，避免 timeout）
> 3. Download Package → 解壓 zip → 在 Android Studio 開啟
> 4. 確認能 build → 產生 APK
> 5. 之後再加 AdMob（JS Bridge 串接原生廣告 SDK）
>
> **公司電腦需要安裝（如果沒有）**：
