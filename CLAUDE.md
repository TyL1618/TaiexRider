...
> 7. ⬜ **使用者 2026-07-10 下班前特別交代要記得的事**：vc19 審核通過、真機裝上
>    新版之後，跑 `adb shell dumpsys package com.tylapp.taiexrider | grep -i AD_ID`
>    確認顯示 `granted=true`，眼見為憑那個 AD_ID 權限真的生效（技術上已經 100%
>    確認生效，這步純粹是使用者想要親眼驗證，非阻塞）。細節見
>    CAPACITOR_EXPERIMENT.md「vc19 已送審」第 3 項。
> 8. 🔴 **必做，且 push 不會自動生效**：`supabase/migration_20260710.sql` 要手動去
>    Supabase Dashboard → SQL Editor 貼上執行（新增 `wallet_daily_usage()`、
>    `wallet_earn()` 加 `granted` 欄位、清掉殘留的舊 `wallet_earn(text)` 重載）。
>    **沒跑 migration 前，前端會自動 fallback 回舊行為（不會壞）**，但「看廣告次數
>    以伺服器為準」跟「今日已達上限提示」這兩個修復不會生效。跑完之後前端要重新
>    打包 vc20 才會帶到玩家手上。背景見下方踩雷筆記「Capacitor 換 origin 導致
>    localStorage 全部歸零」。
>
...
- **不是經濟漏洞**：`wallet_earn` 的每日上限、`consume_attempt()` 的 5 次上限都在伺服器端
  硬性把關，清資料刷不出額外金幣/場次。壞的只有「顯示」跟「沒有提示」。
