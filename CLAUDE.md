...
> 核准狀態 **✓ 就緒**、狀態詳情 **已啟用廣告放送**（不再是 2026-07-28 那時的
> 「廣告放送量受限・審核中」）。這是上一則「四項待辦全部收尾」記錄的**唯一還在跑**的
> 項目，現在也通過了。**TaiexRider 現在是完全就緒、可正式對外營運的狀態**：正式版
> 已公開上線（vc43）、真實廣告 ID 已上線且放送不受限（vc44）、復活排行榜 bug 已修復
> 驗證、prelaunch_cleanup 已執行、IAP 收款已設定。目前沒有已知阻塞項待辦。
>
> ## ✅ 2026-07-28（收工確認）：vc44 已是正式版最新版本，四項待辦全部收尾
>
> 使用者確認以下狀態，記錄下來避免下次換機器又誤判成「還沒做」：
> 1. **vc44（真實 AdMob 廣告 ID + 廣告失敗提示）已打包上傳、審核通過，是目前
>    Play Console 正式版軌道的最新版本**（見上方 2026-07-27 條目的待辦①②已完成）。
> 2. **7/23 修的「復活後排行榜分數被舊分數蓋掉」bug 已真機/PWA 驗證確認修復**
>    （見上方 2026-07-23 第五輪條目，原本標記「強烈建議真機實測」的項目已測過確認正常）。
> 3. **`supabase/prelaunch_cleanup.sql` 已執行**，封測期測試資料（金幣/成就/
>    排行榜等）已清除歸零。
> 4. **IAP 金流（含 Play Console Payments profile 收款銀行帳戶）確認完成**，
>    可正常收真實營收。
>
> **現在唯一還在跑的只剩 AdMob 廣告放送審核**（見上方 2026-07-27/28 條目：
> app-ads.txt 已驗證通過，AdMob 應用程式狀態顯示「廣告放送量受限・審核中」，
> 這是 Google 對新連結 Play 商店 listing 的 App 的正常審核期，非設定錯誤，
> 純粹等待）。**這關過了，TaiexRider 就是完全就緒、可正式對外營運的狀態**。
>
...
>
> 使用者實測回報：玩到 1570 死掉、看廣告復活、繼續玩到 2300，結果排行榜留下的是
> 1570 不是 2300。查 [GameCanvas.tsx](src/game/GameCanvas.tsx) 死亡邏輯發現根因：
> 死亡動畫跑滿 1.5 秒後，程式碼**不管玩家等一下會不會按「看廣告復活」**，就立刻
> 呼叫 `onGameOverRef`（觸發 `App.tsx handleGameOver`——排行榜提交/金幣結算/成就
> 記錄的唯一入口）。但「看廣告復活」按鈕就長在同一張死亡結算畫面上——玩家死掉→
> 分數提早送出→點復活→繼續玩到更高分→真正結束再送一次，兩次提交間隔太短（尤其
> 網頁版看廣告是 `Promise.resolve(true)` 直接過關，復活幾乎瞬間）常撞到
> `submit_daily_score()` 的 10 秒提交冷卻，較高分的第二次被靜默擋掉，只留下復活
> 前的低分。**同一個回呼也負責發金幣/記錄成就，代表復活一次可能被當成兩趟結算，
> 存在重複發放的疑慮**（未逐項實測金幣數字，但邏輯上是同一個漏洞）。
>
> **✅ 已修**：新增 `hasRevived`/`gameOverFired` 兩個遊戲迴圈內部旗標 +
> `finalizeCrash()`（透過新增的 `finalizeCrashRef` 橋接給迴圈外的「返回排名賽」
> 按鈕/實體返回鍵用，跟既有 `reviveSignal` 是同一種 ref 橋接手法）。死亡當下如果
> `revivalEnabled && !hasRevived`（還有復活機會沒決定），**不會**立刻結算，改成
> 等玩家真的離開（點「返回排名賽」／按返回鍵）或用掉復活後再次死亡/完賽時才觸發，
> `gameOverFired` 確保整趟賽局的排行榜提交/金幣結算全程只執行一次。完賽路徑
> 同步補上 `gameOverFired = true`，避免完賽後又點到「返回排名賽」重複觸發。
>
> typecheck/build/`cap sync android` 全過。**這是遊戲核心迴圈的結算時機邏輯，
> preview 環境沒辦法完整驗證**（daily 模式復活需要登入才能挑戰，且 preview 隱藏
> 分頁 rAF 凍結跑不完整局，跟本檔記錄過的既有環境限制一樣）——程式碼已仔細追根因
> 逐一確認呼叫路徑（doRevive/finish/crash 三處呼叫點都檢查過），非臆測性修改，
> 但**強烈建議真機或 PWA 網頁版實測一次**：死掉→復活→繼續玩到更高分→完賽或
> 再摔一次，確認排行榜/金幣最終只算最後一次的正確結果。**使用者已確認 vc41 打包
> 上傳過了，這批 versionCode 推進到 42／versionName "1.42"**。**使用者待辦**：
> ① 下次重新打包 Android vc42（PWA push 後網頁版立即生效）；② 真機/PWA 走一次
> 復活流程確認排行榜分數/金幣沒有被重複結算或用舊分數蓋掉。
>
> ## 📦 2026-07-23（第四輪）：打包前確認——versionCode 推進到 41
>
> 使用者要打包今天累積的三輪改動（計分邏輯修復＋抽獎診斷原因碼、幸運轉輪十連抽
> 等體驗回報全修、Cyberpunk 視覺重新設計＋首頁標題流光），打包前重跑一次
> `npm run build` + `npx cap sync android` 確保 `dist/` 是最新（不是沿用可能過期
> 的舊產物，見「踩雷筆記」§Capacitor 打包前一定要 npm run build），確認 9 個外掛
> 都在、typecheck 乾淨。`versionCode` 40→**41**／`versionName "1.41"`。**使用者
> 待辦**：Android Studio 打包簽署版 AAB 上傳 vc41（今天四輪的 SQL migration
> `20260723.sql`/`b.sql`/`c.sql` 使用者已在稍早跑過，這裡不重複列待辦）。
>
> ## 🎨 2026-07-23（第三輪追加）：幸運轉輪 Cyberpunk 視覺重新設計 + 首頁標題流光
>
> 使用者對幸運轉輪按鈕「沒美感、太大」不滿意，來回好幾輪用 HTML mockup 討論定案
> （切角→使用者回報跟遊戲其他圓角物件不搭→改圓角；瞄準框裝飾→使用者看不懂是
> 什麼設計→拿掉；流光速度→使用者覺得太快→放慢；中獎線對齊問題→查出是 mockup
> 疊層 padding 的示範誤差，非真實邏輯問題），最終方向：**全部圓角**（不做切角/
> 直角，跟遊戲既有語言一致）+ **conic-gradient 流光邊框**（緩慢旋轉，不同面板
> 各自放慢到不同秒數：按鈕 5s／轉輪面板 11s／結果彈窗 7s，避免太吵）+ **按下去
> 顏色互閃**。同時使用者覺得首頁太無聊，追加標題流光。
>
> **✅ 已接進真代碼**：
> 1. `LotterySlot.css` 整份重寫：`.lottery-spin-btn`（單抽/十連/免費抽獎共用
>    基底，`--lc` 自訂屬性決定青/琥珀主色）、`.lottery-machine`（轉輪面板）、
>    `.lottery-winline`（中獎線，金→青）、`.lottery-result`/`.lottery-x10-result`
>    （中獎彈窗，紫金配色呼應稀有車款）全部改用同一套技法：`@property --lottery-
>    spin` 註冊可動畫的角度自訂屬性 + `conic-gradient(from var(--lottery-spin))`
>    疊 `mask-composite: exclude` 只顯示邊框環那一圈，`animation` 讓角度從
>    0→360deg 無限循環＝邊框持續旋轉的流光效果。稀有度特效（rare/epic）原本靠
>    `border-color` 換色，但邊框已經沒有真正的 `border`（改遮罩技法），改用
>    `--lc` 自訂屬性換流光顏色才能生效，這是這次重寫特別要注意銜接對的地方。
> 2. `LotterySlot.tsx`：新增 `flashPress()` 輔助函式（點擊時直接操作 DOM
>    classList 短暫加 `.flash` 380ms 後移除，純視覺效果不用 React state）套用到
>    三顆按鈕；單抽/十連按鈕內容拆成 `.lottery-btn-main`/`.lottery-btn-price`
>    兩行（原本擠在同一行文字），十連按鈕加「省10」小標籤；鑽石數字改成
>    `.lottery-diamond-chip` 藥丸造型讀數。
> 3. `Home.css` `.home-title`：漸層文字（青→白→青→琥珀→青→白→青循環）用
>    `background-clip: text` 緩慢橫向流動（9 秒一輪）疊加呼吸光暈（4.5 秒明暗
>    一次，兩個動畫週期刻意錯開避免同步顯得躁動），加了
>    `prefers-reduced-motion` 降級（靜態光暈，不流動）。preview 環境本身回報
>    `prefers-reduced-motion: reduce`（沙盒瀏覽器預設值，不是 bug），已用
>    `getComputedStyle`／`matchMedia` 直接驗證降級邏輯正確觸發、漸層/裁切機制
>    本身有效（`background-clip:text` 產生 `color:rgba(0,0,0,0)`）；真正的流動
>    動畫效果需要真機/一般瀏覽器（沒開省電/降低動態效果）才看得到，preview 這
>    點測不到。
>
> typecheck/build/`cap sync android`（9 個外掛都在）全過，preview 額外用
> `document.styleSheets` 直接檢查所有新 CSS 規則（`.lottery-spin-btn`／
> `.lottery-machine`／流光相關）都有正確解析、無語法錯誤被靜默丟棄；抽獎轉輪
> 實際登入後的按鈕/面板視覺效果、首頁標題流動動畫，因既有的「preview 訪客身份
> +可能命中省電模式」限制沒能截圖驗證，**建議真機或 PWA 網頁版實際看一次**。
> `conic-gradient`／`@property`／`mask-composite` 都是現代 Chromium 支援良好的
> CSS 特性，Android WebView（Capacitor 走系統 WebView，近年版本皆為 Chromium
> 核心）理論上完全相容，但因為這批全是純視覺效果，還是建議實機確認一次質感。
> 純前端改動，沒有動到資料庫/伺服器邏輯，**PWA push 後立即生效**，Android 下次
> 打包會自動含進去，不用特別為這個推 versionCode。
>
> ## 🔍 2026-07-23（第二輪追加）：漸進給分改回二選一 + 排行榜提交診斷原因碼
>
> 上一輪（同日第一輪）剛把落地角度計分改成「漸進給分」，使用者實測後回報不想要
> 中間值（+50 這種），要就 0 要就 100。**已改回二選一**：`uprightCosThreshold`
> 從 0.55（≈57°）放寬到 0.34（≈70°）減少誤判，`roughUprightCosThreshold`/
> `partialFlipRatio` 兩個常數移除，`settleFlip()` 回到單一門檻判定（詳見
> [DEVDOC.md](DEVDOC.md) §5.2）。
>
> 同時使用者回報一個更嚴重的問題：**跑了一場 2100 分，排行榜上舊的 1784 沒被
> 覆蓋掉**。查 `submit_daily_score()` 發現這支 RPC 回傳型別是 `void`，內部六道
> 防作弊檢查（冷卻/分數上限/翻轉比例/replay 一致性…）任何一道沒過都靜默
> `return`，前端只看 HTTP 是否 200——靜默 return 一樣回 200，玩家跟開發者都
> 看不出「真的寫入成功」還是「被防作弊擋下」，逐一推演六道檢查也排除不了具體是
> 哪一關（使用者提供的線索：非當天首次提交、非十秒內連續提交、翻了多發、分數
> 屬正常範圍——排除了冷卻/上限這些明顯嫌疑，但沒有伺服器記錄還是無法 100% 確定）。
>
> **✅ 已修（診斷缺口，不是亂猜的修法）**：`supabase/migration_20260723c.sql`
> 把 `submit_daily_score()` 簽章改成 `returns table(ok boolean, reason text)`，
> 每個提早 return 的分支標上明確原因碼，成功路徑拆成 `ok`（真的改善分數）跟
> `not_improved`（新分數沒有比現有紀錄好，正確不覆蓋——既有「分數只增不減」
> 設計的正常結果，不是拒絕）。前端 `leaderboard.ts` 同步解析回傳、非 `'ok'`
> 一律印 console，下次再發生就有實際線索可查。完整記錄見
> [ANTICHEAT_DESIGN.md](ANTICHEAT_DESIGN.md)「診斷缺口修補」段落。
>
> typecheck/build/`cap sync android` 全過。**使用者待辦**：① Supabase SQL
> Editor 跑 `migration_20260723c.sql`（**要排在同日稍早的
> `migration_20260723.sql`/`b.sql` 之後**）；② 下次重新打包 Android；
> ③ 如果分數沒送出的狀況再發生，開瀏覽器 console 或 `adb logcat` 看
> `[leaderboard] submit_daily_score 未寫入新分數，reason=...` 這行，把
> reason 值回報回來就能精確定位是哪一道檢查擋下的。
>
> ## 🎰 2026-07-23：計分邏輯修復 + 抽獎轉輪十連抽 + 一批體驗回報全修
>
> 使用者一次回報翻轉計分異常、抽獎轉輪多項體驗問題，先討論分析（含用 HTML
> mockup 對十連抽排版）定案後全部一次動工，一次 commit。
>
> **🎮 計分邏輯（[GameCanvas.tsx](src/game/GameCanvas.tsx) `settleFlip()`）**：
> 1. **圈數公式改「四捨五入到最近整圈」**（`round(|rot|/2π)`），取代舊版
>    `floor((|rot|+0.3π)/2π)`——舊公式每多一圈要轉滿到下一圈的 85% 才進位，
>    跟「是否觸發完美落地」的判定門檻（固定 1.7π）不同步，玩家轉了明顯超過
>    1 圈、逼近 2 圈的角度會觸發「完美落地」提示卻只用 1 圈的分數結算，體感
>    矛盾（使用者實測抓到這個真實 bug）。
> 2. **落地角度沒對齊不再整包歸零，改漸進給分**：`RULES` 新增
>    `roughUprightCosThreshold`(0，≈90°內)＋`partialFlipRatio`(0.5)，介於
>    「滿分門檻」跟「還算立著」之間打折給分，只有落地角度歪到接近側躺/倒插
>    才 0 分——修「轉夠圈數但落地角度沒對齊，翻轉分整趟被沒收」的挫折感。
> 3. **`shareScore()` 分享文案改讀 `finalScoreRef`**：原本讀節流的
>    `hud.points`，跟結算畫面（早就修過同一個坑）不同步，終點/摔車瞬間可能
>    顯示落後的分數。⚠️ **釐清一個誤解**：真的「完賽」（越過終點線）在物理
>    公式上行進分必定被夾到剛好 1000，分數理論上一定整百結尾；非整百結尾的
>    分數必然來自「摔車」，這條數學結論已跟使用者對過，不是 bug。
>
> **🎰 抽獎轉輪（詳細技術記錄見 [LOTTERY_DESIGN.md](LOTTERY_DESIGN.md) §17）**：
> 1. **🔴✅ 真正的 bug：免費抽獎沒接「已買永久去廣告」判斷**——
>    `LotterySlot.tsx` 從一開始就漏了這段判斷，已買去廣告的玩家點免費抽還是
>    要看廣告，跟車庫/結算畫面既有邏輯不一致。已補上。
> 2. **版面**：`VISIBLE` 7→5（比照 `RandomSlot.tsx` 同一修法），滾輪變矮不再
>    把標題/按鈕/機率表擠到邊緣。
> 3. **轉動中鎖返回鍵**：`App.tsx` 新增 `screenLockedRef` + `onLockBack` 回報
>    機制（沿用集中管理 history 的單一 popstate 入口，不用另掛 listener），
>    鎖定時吞掉返回、「‹ 返回」鈕同步灰掉（新增全站共用 `.back-btn:disabled`）。
>    抽獎結果在動畫開始前就已經到帳，中途離開不會弄丟獎勵，純粹是避免看不到
>    抽到什麼的困惑體驗。
> 4. **轉動中點擊加速**：`LotterySlot.tsx`/`RandomSlot.tsx` 都改用「虛擬經過
>    時間」驅動原本的緩動曲線，點擊時調高速度倍率即可用更快節奏跑完同一套
>    動畫，不是瞬間跳結果；旁邊加「👆 點擊畫面可加速」提示。
> 5. **十連抽（190 鑽石／次）**：新 SQL `lottery_spin_x10()`
>    （`migration_20260723b.sql`）一次扣款、迴圈呼叫抽出來的共用機率函式
>    `lottery_roll_prize()` 10 次（避免機率表以後改動時兩處分岔）；前端只跑
>    一次滾輪動畫、停在這次十連裡最稀有的一項撐住儀式感，其餘 9 個直接開獎，
>    接一個 2×5 小卡網格的結果畫面。單抽/十連按鈕改左右並排，價格直接標在
>    按鈕上。
> 6. **票券全面改名「廣告抵用券」**（純顯示層，程式碼變數/函式名不動）。
> 7. **車庫新增貨幣說明**：金幣/鑽石/廣告抵用券旁加「?」按鈕，點開說明各自
>    用途來源。
>
> **💰 其他**：自選/隨機賽道金幣獎勵完賽 5→10、摔車 2→4（隨機賽道走勢難易
> 不可控，獎勵加碼平衡賭注感，同步改 `wallet_earn()` 伺服器端數字）；首頁
> 「遊玩教學」補上幸運轉輪/個人化裝備/廣告抵用券三個完全遺漏的段落，數字
> 改用強調色（新增 `.log-num` class）避免整篇白字看不出重點。
>
> typecheck/build/`cap sync android`（9 個外掛都在）全過，preview 驗證過
> 首頁教學新段落、車庫貨幣說明彈窗、抽獎轉輪訪客頁、隨機賽道點擊加速觸發，
> 均無 console error（登入相關的完整抽獎/十連抽/返回鍵鎖定效果需真機或 PWA
> 網頁版走一次確認，preview 訪客身份 + 隱藏分頁 rAF 凍結測不到完整流程，
> 屬本檔記錄過的既有環境限制）。**使用者待辦**：① Supabase SQL Editor 依序
> 跑 `migration_20260723.sql` → `migration_20260723b.sql`；② 下次重新打包
> Android（PWA push 後網頁版立即生效）；③ 真機/PWA 實測：十連抽開獎畫面、
> 已買去廣告的免費抽是否真的不跳廣告、轉動中返回鍵鎖定、翻轉計分手感。
>
> ## 👤 2026-07-22：排行榜點暱稱看玩家資料頁——vc40
>
> 使用者提的新功能：每日排名賽排行榜點某列的暱稱，跳出那位玩家的公開資料頁。
> 內容經幾輪討論定案（過程用 HTML mockup 對排版）：完整暱稱＋裝備的稱號/前綴
> 圖示/暱稱顏色、**目前裝備的車子（真車圖）**、持有車款數（12/23 台，不列出是哪些）、
> 每日榜🥇🥈🥉次數＋前十次數、經典榜「逐地圖逐名次＋次數」（例：2008金融海嘯
> 週第一 17 次、週第二 9 次……看得出誰擅長哪張賽道）、已解鎖成就 pill。刻意
> **不放**個人最佳分數（脫離地圖脈絡沒意義）、**不放** Google 頭貼（隱私/合規顧慮，
> 使用者拍板先不做）。
>
> **技術重點**：
> 1. **車皮裝備升級成伺服器權威**（`garage.ts`）：原本車皮「目前裝備哪台」是純本地
>    偏好（`tr_garage_active_{uid}` localStorage），別人看不到、且 2026-07-15 記錄過
>    「換裝置車皮跑掉」。改成跟稱號/顏色同一套 `player_wallet.equipped`（新增 'skin'
>    鍵，`wallet_set_cosmetic` 白名單加 'skin'），`getActiveSkinId` 改讀 equipped 快取、
>    `setActiveSkin` 改 async 走伺服器同步。順帶：syncWalletFromServer 加「一次性沿用」
>    把舊本地值搬上伺服器（避免更新後車皮被重置要重裝），setActiveSkin 仍保留寫舊
>    本地 key 當「PWA 已更新但 migration 還沒跑」過渡窗的安全網。
> 2. **`get_daily_top()` 多回傳 `player_id`**（Supabase auth uuid，非 PII）：排行榜點
>    暱稱要靠它查 profile。`ScoreRow.player_id` 新增（optional，舊快取沒有時該列不可點）。
> 3. **新 RPC `get_player_profile(p_player_id)`**（security definer，anon 可讀）：彙整
>    暱稱/裝備/owned/成就原始計數/每日榜名次次數/經典榜逐地圖逐名次次數。名次次數
>    **直接數既有結算表**（`daily_diamond_settlement`/`classic_diamond_settlement` 本來
>    就逐日/逐週存 rank），不用新表。成就「解鎖與否」由前端用既有門檻常數判斷
>    （`achievements.ts` 新增 `computeAchievements()` 純函式，前後端門檻不重複）。
> 4. 新 `src/lib/playerProfile.ts`＋`src/components/PlayerProfile.tsx`/`.css`；
>    `DailyChallenge.tsx` 排行榜列暱稱可點（有 player_id 才可點，淡虛線提示）開彈窗。
>
> ⚠️ **範圍**：只做每日排名賽排行榜的點擊入口；經典模式排行榜（`ClassicSelect.tsx`
> 版面窄、走不同查詢）這次不加點擊，但 profile 內容本身有涵蓋經典榜統計。名次次數
> 來自「已結算」的歷史（當期進行中的日/週要等結算才計入，這是正常語意）。
>
> typecheck/build/`cap sync android` 全過，preview 每日排名賽頁正常渲染真實排行榜、
> 無 console error（**點擊→彈窗的完整效果需 migration 跑完 + 有資料才驗得到**：
> 現在 preview 抓到的是舊 get_daily_top、暱稱還沒帶 player_id 故不可點，屬預期的
> 過渡降級）。**versionCode 已推進到 40／versionName "1.40"**。**使用者待辦**：
> ① Supabase SQL Editor 跑 `migration_20260722.sql`（跑完排行榜暱稱才會變可點）；
> ② 下次重新打包 Android vc40；③ 真機/PWA 登入實測——點暱稱開資料頁、車皮裝備
> 換裝置能不能同步、每日榜/經典榜名次次數對不對。
>
> ## 🧹 2026-07-22：`prelaunch_cleanup.sql` 補上 `wallet_daily_lottery`
>
> 使用者要求把這陣子新增的資料庫功能（抽獎轉輪/個人化裝備等）全面比對一次
> `prelaunch_cleanup.sql`，看有沒有漏清的表。逐張核對現存 18 張表後，發現
> `wallet_daily_lottery`（`migration_20260721.sql` 每日抽獎免費次數，結構跟
> 已經在清單裡的 `wallet_daily_attempts` 一樣）沒被加進清除清單——已補上
> `truncate table public.wallet_daily_lottery;`。這張表本身有 14 天滾動清理
> （`cleanup_old_wallet_logs()`），不清也不會有資料庫肥大或誤發獎勵的風險，
> 純粹是為了上架當天「歸零」的體驗一致性。其餘新增欄位（`player_wallet.
> equipped`／`player_achievements.total_flips,total_perfect`／`daily_scores.
> cosmetics,skin_id`）都加在既有政策已經涵蓋的表上，不需要額外處理。
>
> **還沒拍板的開放問題**：`player_achievements`（成就進度）目前仍刻意不清，
> 但現在已經跟「空中飛人/地心引力挑戰者/完美落地大師」這幾個看得到的稱號
> 綁在一起，不只是車皮解鎖——正式上架時要不要也一併歸零，待使用者之後決定。
>
> 純 SQL 文件改動，不影響程式碼/typecheck/build。**使用者待辦**：跟原本
> `prelaunch_cleanup.sql` 一樣，留到正式上架當天自行決定時機手動跑。
>
> ## 🎨 2026-07-22：修黑天鵝稱號/徽章裝備無反應 + 個人化裝備加確認彈窗 + 加「無」選項——vc39
>
> 使用者用 SQL 直接把抽獎專屬的 `title:blackswan-witness`（黑天鵝目擊者稱號）/
> `badge:blackswan`（🦢 徽章）灌進自己帳號的 `owned` 測試，回報點擊沒反應裝備不了。
>
> **🔴✅ 真正的 bug**：`Garage.tsx handleCosmeticClick()` 開頭 `if (opt.price < 0)
> return;`（本意是擋「price:-1 的抽獎專屬項目不能被購買」）寫在函式最前面、對所有
> 點擊一律生效，導致**已經擁有、想裝備的點擊也被一起擋掉**，永遠進不到下面的裝備
> 切換邏輯——跟上一輪「成就稱號解鎖後點裝備沒反應」是同一種「用途混用的檢查」錯誤
> 模式。已修：把這個檢查移進「尚未擁有」分支裡，只擋購買、不擋裝備。
>
> **✅ 順便做了兩項使用者提出的 UX 改進**：
> 1. **每次裝備/取消裝備都跳確認彈窗**：原本已擁有項目點擊會直接切換裝備，改成跟
>    購買一樣先跳確認彈窗（複用同一套 `.modal-overlay`/`.slot-result` 樣式），新增
>    `confirmEquip` state（`opt` 為 `null` 代表清空）。「🎯 成就稱號」區塊（連勝
>    狂魔等 5 個進度解鎖稱號）原本是獨立按鈕直接呼叫 `setActiveCosmetic`，一併改走
>    同一個確認彈窗，兩個入口裝備稱號的體驗現在一致。
> 2. **五類個人化裝備（暱稱顏色/稱號/前綴圖示/尾焰特效顏色/鬼影顏色）各加「無」
>    卡片**：技術上清空裝備本來就能做到（點擊「目前已裝備的那項」再點一次會取消，
>    伺服器 `wallet_set_cosmetic` 也早就正確支援 `p_id=null` 清空），但沒有明顯的
>    「無」卡片可以按，玩家不知道要怎麼清空。現在每個分類選項列表最前面固定一張
>    「無」卡片，點下去＝清空該類別（暱稱顏色回到預設白色、稱號欄位變空……以此
>    類推，因為前端所有讀取 `getActiveCosmetic()` 的地方本來就把 `null` 當「用
>    預設」處理，不用額外改動）。
>
> 三項都只動 `Garage.tsx`/`.css`，沒有動到資料庫（伺服器端邏輯本來就是對的）。
> typecheck/build 過，preview 訪客視角車庫頁無 console error（「個人化裝備」區塊
> 只有登入才顯示，實際點擊確認彈窗/「無」卡片的效果需要真實登入才能驗證，跟這份
> 文件記錄過的其他登入相關功能一樣受限於 preview 無法走 Google OAuth）。**純前端
> 改動，PWA push 後立即生效；`versionCode` 已推進到 39／versionName "1.39"**（跟
> 上一則 IAP 換套件的 vc38 分開兩顆版號，不是同一次打包）。**使用者待辦**：①
> 下次重新打包 Android vc39；② 登入帳號實測——黑天鵝稱號/徽章能不能正常裝備、
> 點裝備會不會跳確認彈窗、每個分類的「無」卡片能不能正確清空。
>
> ## 👻 2026-07-22：鬼影顏色改成染色剪影+同款尾焰（方案A+C），取代原本的圓形色塊
>
> 使用者實測後回饋：鬼影顏色目前是疊一顆半透明圓形色塊在車身後面（`GameCanvas.tsx
> drawGhost()`），跟車身輪廓對不上，看起來像一坨顏色而不是「這台車被染色了」。討論
> 三個方案（A 離屏染色剪影／B 拉長光暈／C 同款尾焰粒子）後使用者選 **A+C** 組合。
>
> **✅ 方案A（染色剪影取代圓形）**：新增 `getTintedSprite()`——離屏 canvas 用
> `source-atop` 把顏色只蓋在車身不透明像素上（跟車身輪廓完全貼合），烘一次快取
> （`_tintedSpriteCache`，key=圖片URL+色票），之後每幀純 `drawImage`，不用
> `ctx.filter`/`shadowBlur`（沿用專案一貫避開的 Android WebView Canvas2D 昂貴逐像素
> 操作）。染色版用 `'lighter'` 疊在真實車身**底下**、放大 1.1 倍當一圈貼合輪廓的
> 光暈，真實車身仍照舊半透明疊在最上層（2026-07-15 拍板的「鬼影要秀出對手真實車款
> 當購買誘因」設計沒有被推翻，只是把色塊換成貼合形狀的光暈）。
>
> **✅ 方案C（同款尾焰粒子）**：新增 `ghostTrailParticles` 陣列，跟玩家自己的尾焰
> （`trailParticles`）共用同一個粒子型別/衰減常數（`TRAIL_FADE_SEC`），但鬼影沒有
> 物理迴圈可以掛，改成在 `drawGhost()` 依插值路徑位置，用「跟地形的垂直距離 <4px」
> 近似「貼地」時噴粒子（插值路徑本來就含空中拋物線，這個近似在地面附近夠準、空中
> 不會誤觸發）。透明度比玩家自己的尾焰淡一點（0.5 vs 0.65）避免鬼影尾焰搶戲。
>
> typecheck/build 過，preview 驗證首頁載入無 console error（鬼影實際渲染效果需要
> 真實登入帳號＋排行榜有第一名 replay 資料才會觸發，跟這份文件多處記錄過的既有
> preview 環境限制一樣——分頁隱藏會凍結 rAF、跑不完整局，**建議真機或 PWA 網頁版
> 實際刷一筆第一名再開鬼影開關確認視覺效果**）。純前端渲染改動，沒有動到伺服器/
> SQL/原生殼，**PWA push 後立即生效，不用等下次 Android 打包**（下次不管為什麼原因
> 打包都會自動含進去，不用特別為這個推 versionCode）。
>
> ## 💳 2026-07-22：換 IAP 外掛因應 Google Play Billing Library 8.0.0 強制期限——vc38
>
> Play Console 政策審查跳出警告：**2026-08-31 起所有 App 必須使用 Google Play Billing
> Library 8.0.0 以上版本**，否則無法發布更新。查證發現專案用的
> `capacitor-native-purchases`（jokio 維護）內部寫死 Billing **7.1.1**，最後一次更新
> 停在 2026-02、之後沒有跟進，等於卡死。Billing 7→8 有 breaking change（拿掉
> `queryPurchaseHistoryAsync()` 等 API），不能直接在 Gradle 裡強制指定新版號蓋過去。
>
> **✅ 已修：換成 `@capgo/native-purchases`**（Cap-go 維護中，內部用 Billing
> **8.3.0+**，大版號跟安裝的 Capacitor 8 對齊）。`src/lib/billing.ts` 原生購買三支函式
> （查價/購買/對帳）整個改寫，API 模式從「responseCode 數字判斷」換成
> 「Promise resolve/reject」：
> - `fetchPackPricesNative`：`getProductDetails()` 逐一併發查詢 → `getProducts()`
>   一次查全部（新外掛明文警告 `getProduct()` 不可併發呼叫，會 race condition）。
> - `runPurchaseFlowNative`：`purchaseProduct()` 直接回傳的 `Transaction` 已含
>   `purchaseToken`，不用再像舊版另外呼叫 `getLatestTransaction()` 補查。
> - `reconcilePurchasesNative`：改用 `getPurchases()` 撈「目前有效購買」清單，不用再
>   對每個已知 SKU 個別查詢。
>
> **順便解掉一個舊架構隱患**：舊版 jokio 外掛的 `purchasesUpdatedListener` 對「所有
> INAPP 商品」一律自動 consume，不分消耗型/非消耗型——`remove_ads_forever`（設計上
> 買一次終身有效）也會被自動 consume，導致 Google 端誤判成可以再買一次（當時純靠
> UI 層擋住購買按鈕當防線，不影響安全性但體驗不乾淨）。新外掛的
> `purchaseProduct({isConsumable:false, autoAcknowledgePurchases:false})` 讓客戶端
> 完全不碰 consume/acknowledge，一律由 `verify-iap-purchase` Edge Function 走 Google
> Play Developer API 在伺服器端處理（跟原本架構完全一致，外掛官方文件也建議這樣做），
> 這個問題從架構上直接消失，不用再靠 UI 層防守。
>
> ⚠️ **已知限制（外掛 8.6.4 版本行為，讀過原生 Kotlin/Java 原始碼確認）**：
> `purchaseProduct()` 失敗時，原生端把「使用者自己取消付款」跟「其他真正的購買失敗」
> 共用同一句泛用錯誤訊息（`onPurchasesUpdated` reject 分支一律丟 "Purchase is not
> purchased"），不像 jokio 舊版有明確的 `BillingResponseCode.USER_CANCELED`（=1）
> 可以分辨。已改成統一當「使用者取消」靜默處理（不彈錯誤橫幅），真正的購買失敗只能
> 靠 `console.warn`／`adb logcat` 除錯——這是換套件後的行為退化，之後外掛若補上明確
> 的取消代碼可以再優化。
>
> `verify-iap-purchase` Edge Function、`AndroidManifest.xml` 的 `BILLING` 權限完全
> 不用改（後端驗證邏輯、purchase_token 格式不受外掛換版影響，只是 Manifest 註解裡
> 的外掛名稱字樣順手更新）。typecheck/build/`npx cap sync android` 全過（9 個外掛
> 數不變，1:1 換掉）。**versionCode 已推進到 38／versionName "1.38"**。
>
> **PWA 網頁版完全不受影響**（網頁版本來就不開放購買，走的是 TWA 時代留下的 Digital
> Goods API 分支，這次完全沒動到）。**使用者待辦**：① 下次重新打包 Android vc38；
> ② **Play Billing 必須真機測付款流程**——買一包鑽石、買永久去廣告各測一次確認能
> 正常收到獎勵，另外故意按「取消付款」確認畫面上真的不會跳錯誤橫幅（這是新外掛
> 行為改變的地方，值得特別驗證）；③ Play Console 那顆「申請延期」按鈕這次沒有按
> （已直接換套件解決根本問題，不需要延期）。完整技術細節見
> [DEVDOC.md](DEVDOC.md) §2.7 附註。
>
> ## 🏷️ 2026-07-21（第九輪追加）：新增 8 個股市梗稱號 + 財經皓角改名
>
> 使用者新增可購買稱號：本多終勝/巴膩膩/軋空逃生/四貸同堂/奈米戶/梭哈是種智慧/
> 逢低買進/公園長椅居民（皆 200 鑽，同既有股市梗稱號價），並把「財經皓角」改名
> 「皓哥黃段子聽眾」（沿用同一個 id `title:finance-haojiao`，只改顯示文字，已購買
> 玩家不受影響）。已改 `Garage.tsx COSMETIC_CATALOG`／`garage.ts COSMETIC_LABELS`／
> `supabase/migration_20260721l.sql`（`wallet_spend_item()` 白名單新增 8 筆）。
> typecheck/build/`cap sync android` 全過。完整記錄見
> [LOTTERY_DESIGN.md](LOTTERY_DESIGN.md) §16。**使用者待辦**：Supabase SQL
> Editor 跑 `migration_20260721l.sql`；下次重新打包 Android。
>
