...
> **2026-07-04 全面盤點**：[FABLE5_HANDOFF.md](FABLE5_HANDOFF.md) 清單裡 debug/監控/內容改善/原生體驗/拉霸音效/OG分享/留存規劃/android三項/上架檢查/ASO **全部已完成並上線**；[BETA_FEEDBACK.md](BETA_FEEDBACK.md) 的 #1/#2/#3 也都已解決。**唯一還沒動工的是反作弊**——見下方「真正還沒做的事」。
...
1. **反作弊機制（唯一真正「尚未動工」的大項）**：設計已在 [ANTICHEAT_DESIGN.md](ANTICHEAT_DESIGN.md) 定案多時，使用者 2026-07-03 明確拍板要做、**分派給 Fable 5 負責**、尚未寫一行 code。三階段：A（RPC 物理一致性驗證＋冷卻，隨時可上）／B（DB 端次數上限＋離群偵測，正式上架後）／C（操作事件序列＋Ghost，跟留存規劃的 Ghost 回放一起做）。
2. **P 系列付費車款（5 台）**：唯一還沒生圖的車款批次，[GARAGE_DESIGN.md](GARAGE_DESIGN.md) §2 prompt 已備妥；生圖後還要接 Google Play Billing（IAP）才能真正販售，Garage.tsx 目前是「敬請期待」UI 殼。
3. **RETENTION_PLAN 第二批**（經典模式 Top N/百分位、經典週榜、週任務、股票圖鑑、狂暴盤日事件）：**等使用者點頭 schema** 才能動工，目前完全沒開始。
4. **RETENTION_PLAN 第三批**（週聯賽分組、Ghost 回放、排行榜 emoji 反應、好友邀請）：長期規劃，工程量大，未排入近期。
5. **BETA #4（前翻/煞車鈕操控）**：未實作，中優先級。
6. **Web Push 通知**：中期項目，工程量較大，未開始。
...
- **反作弊實作**：**尚未做**——詳見文件開頭「## 待辦」→「🎯 真正還沒做的事」第 1 項，避免重複維護同一件事的兩份說明。
...
- [x] **自動去背取代手動去背** 已完成（v0.12.26 已推，2026-07-04）。使用者建了 `public/bikes/Grok_Original/` 存 Grok 白底原始 JPEG（含新生出的 P1/P2），反映手動去背在輪胎底部擦陰影時會留下鋸齒不平整。改寫自動去背腳本（量完即刪，未進版控）：四角採樣背景色→色彩距離算 alpha（smoothstep 平滑非硬切）→ median 濾雜點+輕微 blur 抗鋸齒→邊緣去污染還原真實前景色（防白邊色暈）→ **車輪以下區域用嚴格門檻單獨處理**（車底陰影本質是背景色均勻變暗版，色彩距離跟深黑輪胎輪廓可明確分開，陰影因此整片判定回背景全透明）。開發過程踩了一個 sharp 的雷：`.median()/.blur()` 對單通道 raw buffer 會自動轉成 3 通道輸出，若沒接 `.toColourspace("b-w")` 強制轉回單通道，讀 buffer 會位移錯位整張花掉。B1/B2/Q1/Q2/Q3 五台重新處理（游戲內 520px 版+首頁展示用 900px hires 版都重新產生），肉眼比對＋alpha 數值抽查確認雜訊/陰影乾淨清除、輪子邊緣無白邊；**spriteW/offsetX/offsetY 數值不變**（同一張原始構圖，只是邊緣質量提升，輪位沒有移動）。P1/P2 也一併處理好 hires 版本備用（尚未登記進 BIKE_SKINS，等定價/Billing 決策）。`public/bikes/Grok_Original/` 已加進 `.gitignore`，比照 `raw/` 不進版控。
