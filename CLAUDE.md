...
### 🔖 交接（2026-06-16 晚，死亡 bug 修正 + 地形三角梯形零縫隙）
...
- **地形碰撞體（零縫隙⭐新）**：每段地形拆成兩個三角形，相鄰段共用精確頂點，數學上零縫隙，根治穿落問題。用 `Bodies.fromVertices` 建立，`yBottom = maxY+600` 延伸到螢幕外。
...
- **死亡判定（⭐新邏輯）**：分兩條件獨立判斷：① `翻倒>120° && 有地面接觸 && 速度<5.0` → 翻倒貼地 1 秒死；② `雙輪離地 && 速度<0.5` → 空中卡死 1 秒死。正常飛行中速度 ≈ 6.9，空翻中沒有地面接觸，兩條件均不觸發。
- `public/bike.png` 已就位（610×409 去背），貼圖生效。

**待使用者試玩確認（taiexrider.pages.dev）：**
1. 空中飛行不再死亡？（新死亡條件：需有地面接觸 OR 速度<0.5）
2. 地形接縫穿落已解決？（梯形三角，請讓朋友家人幫忙確認）
3. 重力 0.5 + 速度 6.912 手感？
4. 完美落地在陡坡/平路都能正確觸發？
5. 翻倒貼地 1 秒可以正確判死？
6. 首頁「ℹ v0.3.1」按鈕能看到更新日誌？
