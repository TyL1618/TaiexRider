...
### 🔖 交接（2026-06-16 v0.4.1 — 填滿地形 + discussion 14 點處理）

**開工第一件事：`git pull`。**

> **真機試玩回饋（已修，v0.4.1）**：使用者真機確認「整體非常像 Rider、流暢、K 棒風格 OK、返回邏輯正確」。修掉兩個 bug：
> - **卡 K 棒縫隙（高處落下偶發）**＝Matter.js internal-edge 卡頓。修法：梯形**底部兩角各外擴 `overlap=segmentWidth`**（上窄下寬），相鄰梯形在接縫正下方重疊成實心聯集 → 消除外露垂直內部邊。node 實測峰/谷 union 頂面與折線誤差=0，手感視覺不變。**chassis mask=0 備援未啟用（外擴已解決）。**
