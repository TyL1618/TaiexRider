...
### 🔖 交接（2026-06-15，遊戲內容暫定，可開始 Phase 3）

**最新 commit** ≈ `9835ec9`。開工第一件事：`git pull`。

> 遊戲核心物理/手感已由使用者確認凍結，Phase 2 收尾項目（賽道長度/更多股票/配色）留待後續，**優先推進 Phase 3（三模式 UI）**。

**物理 / 手感現況（已定案，勿隨意改動）：**
- 操控：著地 + 按住 → 沿坡恆速 `cruiseSpeed=9`（Rider 風格）
- 重力：`engine.gravity.y = 1`；後翻 `airSpinMax=0.12 / airSpinAccel=0.006`
- 著地 AV 夾：`|av| > 0.28` 才介入（溫和版，不壓正常跟坡旋轉）
- 車身：52% 縮比（chassisW 48、wheelRadius 6、chamfer 2）
- 坡度：`maxSlopeDeg = 55`（每段最大高差 136px）
- 計分：行進分 0~1000（即時）＋ 後翻/完美落地特技分
- 死亡：`chassisContacts > 0 && 兩輪皆離地`，持續 **2.0s** 判死
- 渲染：shadowBlur 全換雙描邊，掉幀已解決
- **物理插值**（2026-06-15）：`step()` 前存 prev 位置，`render(alpha)` 用 `alpha=acc/STEP` 插值
  drawBike/drawWheel 全改吃插值座標，消除固定步進造成的車身鋸齒卡頓感
- **空中車頭前傾**（2026-06-15）：空中放開時施加 `+airNoseDive=0.006/step`，上限 `airNoseDiveMax=0.10`
  模擬車頭配重，下墜時自然低頭，landing 更容易回正
