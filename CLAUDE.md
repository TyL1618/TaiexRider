...
>
> **⚠️ 使用者待辦**：① Supabase SQL Editor 依序跑 `migration_20260721b/c/d.sql`
> （主檔 `.sql` 已跑過）；② 黑天鵝 `public/bikes/hidden-blackswan.png` 美術尚未
> 產出（`garage.ts` 已登記路徑，圖到位前車庫該卡片剪影會是破圖，不影響其他
> 功能，風格定案「黑天鵝」黑金神秘配色，交給 Grok 出圖即可）；③ 下次重新打包
> Android 才會生效（PWA push 後網頁版立即生效）。
