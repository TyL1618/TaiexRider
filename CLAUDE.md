...
- **落地/對齊貼地**：著地角速度朝坡段比例修正（`groundAlignGain=0.3`，夾 `groundedAvMax=0.10`）。**注意 `groundedAvMax` 壓低關鍵**：太高(0.28)會去「狂追到不了的目標角度」→ 車身狂轉翻過頭→往後甩。輪子驅動時只給切線分量(不保留垂直)→更貼地；`restitution=0.05`。
