...
### 🔖 交接（2026-06-17 v0.7.1 — Google One Tap 登入 + 每日地圖全台股）

**開工第一件事：`git pull`。**

> **v0.7.x 本次完成**：
> - **Google One Tap 登入**：`signInWithGoogle()` 改用 One Tap（nonce 為 SHA-256 hex → GSI；rawNonce base64 → Supabase `signInWithIdToken`）；GSI 封鎖時自動 fallback redirect。首次登入自動帶入 Google 顯示名稱為暱稱。
> - **DailyChallenge 排名賽頁**：未登入顯示 Google 登入按鈕；已登入顯示「以 [暱稱] 參賽」；設定 ⚙ 可改暱稱或登出。排名賽成績提交需 Google 登入（`auth.uid()` 伺服器端驗證）。
> - **OAuth redirect 返回誤彈修正**：`Home.tsx` popstate 加 `isOAuthReturn` 偵測，壓制 OAuth 返回後的第一次 popstate，避免「是否離開遊戲」誤跳。
> - **每日地圖全台股**：GitHub Actions 每日 21:05 抓全台上市股（~1000 支）存 Supabase `daily_map`，難度最高者為當日地圖。已實測全量跑完無錯誤。
> - **排行榜快取**：`fetchDailyTop` / `fetchHardestDailyMap` 加 Promise 快取，進 DailyChallenge 頁面時零等待。提交成績後自動清除當日快取。
> - **地形縫隙修正（v0.7.0）**：梯形頂面兩端各 +3px（`topExtra=3`），相鄰段重疊 6px，消除機車從特定角度落下卡進縫隙的 bug。

> **🟠 下一步選項**：
> - Phase 5：PWA 離線快取（Service Worker + IndexedDB）
> - Phase 6：視覺/音效打磨
> - Phase 7：TWA 包裝 + Google Play 上架
> - 未來：ETF 含字母代號（00981A 等）納入每日地圖（目前篩選條件 `/^\d{4}$/`，改 `/^\d{4}[A-Z]?$/` 即可）
