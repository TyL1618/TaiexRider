...
### 🔖 交接（2026-06-17 v0.6.1 — Phase 4 防作弊 + 地形/翻車修正）

**開工第一件事：`git pull`。**

> **v0.6.1 本日修正**：
> - **防作弊（anti-cheat）**：`submit_daily_score` RPC 移除 `p_date` 參數（改用伺服器 `current_date`，防止指定任意日期）；加入合理性驗證（分數 0~50000、時間 5s~2h、flips/perfect 0~50、player_id 強制 UUID 格式）；前端 `leaderboard.ts` / `App.tsx` 同步移除 `challengeDate` 傳入。Supabase SQL 已重跑，偽造資料已清除。
> - **非對稱坡度**：上坡 55°、下坡 75°（v0.6.0 地形修正）。
> - **翻車瞬間定格**：`topHit` 觸發後立刻 `setStatic` 凍結車身（v0.6.0 修正）。
> - **注意**：多帳號洗榜（不同 player_id spam）仍可能，根治需 Google 登入（Phase 5 後）。
...
> - **成績提交**：`GameCanvas` 新增 `onGameOver?(stats: GameOverStats)` callback；`App.tsx` 判斷 `isDailyRun` → 呼叫 `submitDailyScore`。
...
> - 或繼續補 Phase 4 強化：Google 登入綁定（根治多帳號洗榜）、後端權威下發每日地圖
