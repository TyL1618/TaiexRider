...
- **核心物理改成定速模型（Rider 風格）**：著地時直接 `Body.setVelocity` 鎖定 `cruiseSpeed`(7.5)，不再用 force/torque 驅動 → 解決上不去、卡頓、wheelie 後翻三大痛點。單指唯一作用＝空中後翻（著地按不按都一樣）。**這是對先前「油門驅動」設計的徹底取代**，DRIVE 參數已大改（見 constants.ts）。
- **賽道還原原始折線**（Catmull-Rom 平滑版已砍除，因效果不佳）。
- **手機反藍修正**：index.css 加 `-webkit-tap-highlight-color: transparent`。
- **UI 重構**：右上「返回主選單」、左上設定 icon（音量待實作/顯示版本）、移除 R 鍵、overlay 改「再玩一次」、底部提示精簡。
- **首頁**：排序鈕（熱門/困難度/股號）、搜尋框（限預抓股票）、困難度星級、「昨日盤線」→「前次日盤」。
- ⚠️ **仍待真人試玩確認**：`cruiseSpeed` 速度感、空中後翻手感、折線在定速下過陡段是否仍順。
