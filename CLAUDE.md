...
>
> **③ 現在進度卡在哪、接下來要做什麼**（依序）：
> 1. ✅ Google Cloud Console 已建好 2 顆 Android OAuth Client（Play 簽署金鑰
>    SHA-1＝`87:74:F0:B1:43:BD:43:C3:47:E2:20:C4:5A:D0:AA:DC:63:CF:14:64`、上傳
>    金鑰 SHA-1＝`6B:18:63:B2:BB:59:D4:F6:13:43:CC:1B:DD:B2:01:57:AF:3B:7C:4C`，
>    package 都是 `com.tylapp.taiexrider`）。⏳ 生效中，家裡電腦接手時應該早就
>    生效完了，可以直接測登入，不用再等。
> 2. ⬜ **debug build 測登入**：家裡電腦 `cd android && ./gradlew assembleDebug`
>    （⚠️ 若遇到 AAPT2 莫名崩潰，先檢查防毒軟體有沒有跳警告，不是 Gradle daemon
>    問題，見 CAPACITOR_EXPERIMENT.md 踩雷段），裝到測試機測登入。
> 3. ⬜ 用 Android Studio「Generate Signed Bundle/APK」（用
>    `taiexrider-release.jks`）打一份**簽署版 APK**（不是 AAB），側裝測試：
>    ①登入（驗證上傳金鑰那顆 OAuth Client）②**Play Billing 真實購買流程**（現在
>    applicationId 是正式的，Play Console 既有商品應該查得到了，用授權測試帳號
>    測不會真的扣錢）。
> 4. ⬜ 三座橋在簽署版上都測過沒問題 → AdMob 廣告單元 ID 換真實值（目前仍是
>    Google 測試單元，`src/lib/ads.ts` 的 `NATIVE_AD_UNIT_IDS`）。
> 5. ⬜ 打正式 **AAB** → 上傳 Play Console（不可逆，動手前務必再跟使用者確認一次）。
> 6. ⬜（選配、不急）App 長按捷徑／開屏 Splash Screen 目前還沒搬到 Capacitor，
>    非阻塞，見 CAPACITOR_EXPERIMENT.md「已知功能落差」。
>
> **④ 其他背景**：vc16（TWA 版）已上傳 Play Console 封測、仍在跑審核，**跟
> Capacitor 進度是兩條獨立的線**，不受今天的架構切換影響，繼續等結果即可，不用
> 特別去管它。**vc17 起不會再出 TWA 版**——之後的新版號都會建立在 Capacitor 之上。
>
> 完整細節、踩雷紀錄、決策脈絡全部在
