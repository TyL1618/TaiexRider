...
1b. **🐛 登出/切帳號不刷新 bug**（2026-07-04 晚發現，尚未修）：`signOut()` 沒清 `garage.ts` 的
    localStorage 錢包快取（金幣/鑽石/擁有清單/裝備車皮），登出後首頁仍顯示舊帳號數值；切換
    Google 帳號（不透過登出）app 完全偵測不到。根因分析與修法詳見
    [NEXT_BATCH_PLAN.md](NEXT_BATCH_PLAN.md) 批次 1，7/6 起處理。
