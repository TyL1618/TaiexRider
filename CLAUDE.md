...
- **封測回饋待辦**：見 [BETA_FEEDBACK.md](BETA_FEEDBACK.md)（#3 地圖選取邏輯、#1 波動率動態地形為高優先；#4 前翻、#2 多車型為中——多車型已於 2026-07-03 升級成完整車庫系統規劃，見下）。
...
- **車庫系統**（BETA #2 升級版）：軟通貨＋10 台車皮（2 基本/3 任務解鎖/5 付費），設計與 AI 生圖 prompt 全文見 [GARAGE_DESIGN.md](GARAGE_DESIGN.md)。使用者會自行把該檔內容交給 Grok 生圖。
- **原生體驗補強批次**（使用者要求「除了完美落地慢動作，其他全部都要」）：
  1. **Wake Lock**：遊戲中防止螢幕變暗/鎖屏（`navigator.wakeLock`，需 feature detection + 分頁隱藏時釋放/恢復）
  2. **CSS 破綻修補**：`user-select:none`、`-webkit-tap-highlight-color:transparent`、遊戲區 `contextmenu` preventDefault，消除長按選字/藍色高亮閃爍
  3. **畫面轉場動畫**：首頁↔模式↔遊戲之間加 150-250ms 滑入/淡入；按鈕按下 scale 0.96 回彈
  4. **結算畫面 juice**：分數數字滾動遞增、統計逐項彈出、破 PB 徽章有進場動畫（取代現在整版瞬間出現）
  5. **Web Push 通知**（中期，需接 FCM）：每日新賽道開放提醒、streak 保護提醒
  - ❌ **明確排除**：完美落地慢動作/hitstop（使用者不要這項）
- **全站盤勢主題氛圍**：大漲紅／大跌綠／平盤現有霓虹配色，依前次收盤狀態切換，設計見 RETENTION_PLAN.md「盤勢事件化」段落。
- **反作弊實作**（設計已在 [ANTICHEAT_DESIGN.md](ANTICHEAT_DESIGN.md) 定案多時，**使用者 2026-07-03 明確拍板要做**，尚未寫一行 code）：照文件 Rollout 表分三階段（A：RPC 物理一致性驗證＋冷卻，隨時可上；B：DB 端次數上限＋離群偵測，正式上架後；C：操作事件序列＋Ghost，與留存規劃的 Ghost 回放一起做）。
