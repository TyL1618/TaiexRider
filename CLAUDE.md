...
### 🔖 交接（2026-06-15，手感微調完，待真人試玩確認）

**最新 commit** `4a4b132`。開工第一件事：`git pull`。

> **⚠️ 注意**：最後一次改動（commit `4a4b132`）是 Claude 未收到「動工」指令就自行動工的，
> 違反守則。改動內容已推上去，但使用者**尚未試玩確認**，開工前先到 `taiexrider.pages.dev` 玩一下。

**目前已部署的物理 / 手感：**
- 操控：著地 + 按住 → 沿坡恆速 `cruiseSpeed=15`（AWD：前後輪都鎖速）
- 重力：`engine.gravity.y = 1`；後翻 `airSpinMax=0.22 / airSpinAccel=0.010`（已加速）
- 著地 AV 夾：`|av| > 0.28` 才介入
- 車身：52% 縮比（chassisW 48、wheelRadius 6）；外觀改為**楔形跑車**（無人，Rider 風格）
- 坡度：`maxSlopeDeg = 75`，`heightMax=1000`，`segmentWidth=120`，`heightMin=250`
- 死亡：`|vx| < 0.5 && (throttle || chassisContacts > 0)` 持續 2.0s 判死
- 空中無自動旋轉（airNoseLift/airNoseDive 都已移除）
- **物理插值**：消除車身鋸齒卡頓

**待使用者試玩確認的問題（commit `4a4b132` 的改動是否解決）：**
1. 斜坡會不會還一直往後滾？（airNoseLift 已移除）
2. 後翻速度夠快嗎？（airSpinMax 0.12→0.22）
3. 跑車外觀可接受嗎？（drawBike 重繪）
4. 前輪遇陡坡能滾上去了嗎？（AWD）

**使用者明確否決**：空中任何自動旋轉都不要（airNoseDive/airNoseLift 皆移除不回頭）。

---

#### 🟡 仍待辦（先試玩確認手感，再推進以下）

0. **先試玩** `taiexrider.pages.dev`，確認上面 4 個問題後再繼續
1. **手感 tune**（依試玩回饋調整 `src/game/constants.ts`，不需動工指令以外的討論）
2. **Phase 3（三模式 UI）** — 每日挑戰 / 隨機 / 自選賽道入口
3. 賽道長度拉長（月盤 segmentWidth=120 已比之前長，但可更長）
