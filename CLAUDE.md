...
- **驅動（坡面切線鎖速）⭐ 核心模型**：著地按住 → 取「後輪→前輪連線方向（坡面切線，tx 永遠 > 0 = 恆朝前）」的速度分量，ease 到 `cruiseSpeed=7.2`（`groundLockEase=0.7`，~3步收斂）。**任何坡角同速**；凸坡頂離地時有向上動量 → 飛更遠；tx 永遠正值 → 不倒退。
- **法線速度歸零（吸地消彈跳）⭐ 新加**：著地時每步把「垂直坡面朝外」的速度分量歸零（法線 = (ty,-tx)，只移除 vn>0 的離坡分量）。等效 restitution=0，消除 Matter.js 碰撞微彈 → 輪子不再上坡彈跳 → groundedStreak 正常累積 → launchBoost 可正常觸發。
- **低重力**：`engine.gravity.y = 0.7`（原 1.0）→ 空中時間加長，翻轉窗口更寬（Ketch Rider 風格）。
- **速度解耦（boost 已鎖死）**：地面 `cruiseSpeed=7.2`；離地拉到目標速 `cruiseSpeed×launchBoost≈12`、設上限永不超過。**boost 觸發條件：有按油門 + 速度朝前(vx>0) + 地面連續待夠 `minGroundedStepsForBoost=5`**。
- **離地歸零殘留角速度**：消除爬坡貼坡帶上來的「莫名往後翻」。
- **空中操控（bug 修正）**：按住＝後空翻（`airSpinMax=0.192`、`airSpinAccel=0.024`，移除 `cos(angle)>0` 條件 → 任意角度均可持續旋轉，修「倒置區間停轉/忽快忽慢」bug）；放開＝立刻清除後翻慣性（`Math.max(0,av)` 拔掉負向角速度）→ 再微微前壓（`airNoseForwardAccel=0.0006`、`airNoseForwardMax=0.008`），修「放開後繼續轉」bug。
- **前壓配重**：前輪 `frontWheelDensity=0.0030` > 後輪 `0.0012`。
- **落地/對齊貼地**：著地角速度朝前輪坡段比例修正（`groundAlignGain=0.3`，夾 `groundedAvMax=0.15`）；`restitution=0.05`。
- **地形**：`segmentWidth=80`、`heightRange=420`、`refPct=0.022`；折線**維持原汁原味不平滑**（Catmull-Rom 暫不做）。
- **完美落地規則**：先在空中完成翻滾(`flips>0`) + 真實跳躍 + 正立 + 車身與坡面夾角 < `perfectLevelRad=0.7`(≈40°，已從 0.5 放寬)。用「平行」取代「同 step 雙輪觸地」避免毫秒時間差。
- **地形碰撞接縫修正**：`buildTerrainBodies` 矩形改沿「線段法線往下」偏移半厚度（原本用世界 Y 偏移→斜段頂面橫移/下沉→山峰接縫小台階卡住小前輪）。修「前輪卡山峰、出發頓住」。
- **結算畫面**：`.overlay-result`（透明）讓出中段。進結算 → HUD 全隱藏。結算顯示：標題 + 股票名稱 + 分數 + 時間 + 按鈕（`padding: 18% 0 16%`）。
- **結算迷你股價圖**：canvas 在螢幕 33%~70% 高度畫獨立折線圖（直接用 `prices[]`，比例與物理世界無關）→ 視覺跟 Google Finance 走勢圖一致。漲=紅/跌=綠/平=青，含底部漸層填色。
- **完賽停車**：完賽瞬間 `Body.setStatic` 凍住車身；`doReset()` 解凍。車體隨鏡頭縮放等比縮小（sprite/wheel 尺寸 × camera scale），不再遮住結算視角。
- **V 谷平底**：`pricesToTrack` 後處理：夾角 < 90°（h1×h2 > segW²）的谷底插入 `flatBottomW=80px` 平段，讓車有地方轉向再爬坡；顏色=平盤青。
- **HUD 距離標籤**：改為在股票名稱下方另起一行垂直排列（`{label} {name}` / `距離 {hud.distance}m`）。
...
1. 上坡不再彈跳？行進速度穩定不一陣一陣？
2. 陡坡能飛高了嗎？（groundedStreak 正常 → boost 觸發）
3. 空中時間夠長、翻轉容易控制？（gravity=0.7）
4. 後空翻全程旋轉速度穩定？（倒置區間不再停頓）
5. 放開後旋轉立刻停止？落地更容易控制？
6. 落地仍順暢不彈飛？
