...
export type RewardedAdKind = "coin" | "revive" | "lottery" | "ticket";
...
const NATIVE_AD_UNIT_IDS: Record<RewardedAdKind, string> = {
  ...
  // 抽獎轉輪的免費抽獎、看廣告換票券（LOTTERY_DESIGN.md）都沿用 coin_reward 這組
  // 廣告單元，不另外在 AdMob 開新單元——都是「看廣告換遊戲內獎勵」的同類型版位。
  lottery: NATIVE_TEST_REWARD_AD_UNIT_ID,
  ticket: NATIVE_TEST_REWARD_AD_UNIT_ID,
};
