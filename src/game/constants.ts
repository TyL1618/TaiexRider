...
export const RULES = {
  ...
  // 2026-07-23：曾短暫改過「漸進給分」(角度沒對齊打折而非歸零)，使用者實測後
  // 回報不想要 50 分這種中間值、寧可維持非0即100，已改回二選一，門檻順便從
  // 57° 放寬到 ~70°（0.34）減少「明明轉夠圈數卻卡在門檻外整包歸零」的誤判。
  uprightCosThreshold: 0.34, // cosθ > 此值算正立 (≈ 70°內)，過門檻給滿分，沒過 0 分
