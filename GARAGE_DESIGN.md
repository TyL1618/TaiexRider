...
> **狀態更新（2026-07-04）：手動去背改自動化腳本，車底陰影橢圓也一併自動去除**。使用者反映手動去背在輪胎底部擦陰影時會留下不平整/不圓滑的鋸齒邊緣。改成把 Grok 生成的原始 JPEG（白底、未去背）存進 `public/bikes/Grok_Original/`（已 `.gitignore`），Claude 寫自動去背腳本（量完即刪，未進版控）：四角採樣背景色 → 色彩距離算 alpha（smoothstep 平滑漸變非硬切）→ median 濾椒鹽雜點 + 輕微 blur 抗鋸齒 → 邊緣去污染還原真實前景色（避免白邊色暈）→ 車輪以下區域用嚴格得多的門檻值單獨處理（車底陰影本質上是背景色的均勻變暗版，跟深黑輪胎在色彩距離上可明確分開，陰影因此被判定回背景全透明，輪胎輪廓不受影響）。B1/B2/Q1/Q2/Q3 五台全部重新處理，肉眼比對＋透明度數值驗證乾淨（過程與細節見 CLAUDE.md 進度）。**此流程比先前「Grok 生圖→使用者手動去背」更穩定可重現，之後 P 系列生圖後直接照這套流程處理即可，不用再手動去背**。
...
### ✅ B1/B2/Q1/Q2/Q3 五台都已上線，下一批是 P1~P5（付費款）

Q1 多頭鬥牛已於 2026-07-03 深夜補生成功（過程見文件開頭狀態更新：第一次撞到生圖模型
記憶體洩漏跑出圖庫浮水印、第二次側箱擋輪，第三版禁側箱+chart 移到油箱才成功）。
**目前只剩 P1~P5 五台付費款還沒生圖**，之後要生的話直接用下面「P1~P5」那幾段 STYLE
（在共用規格之後）；Q1 的 STYLE 段落已更新成最終成功版，留著備查不用再動。
...
1. Grok 產出的白底原圖（未去背）存進 `public/bikes/Grok_Original/`。**2026-07-04 起改自動去背，不用再手動用工具擦**：Claude 寫的自動去背腳本（四角採樣背景色→色彩距離算 alpha→median+blur 抗鋸齒→邊緣去污染→車輪以下區域嚴格門檻連車底陰影一併去除）處理成透明 PNG 存進 `public/bikes/raw/`。這套流程比手動去背更穩定、邊緣更平滑（尤其輪胎底部），量完即刪的腳本未進版控，需要時跟 Claude 說一聲重新寫。
