...
- [x] **鑽石購買頁骨架 + Google Play Billing 串接** 已完成程式碼（2026-07-06，v0.12.31，
  ...
  **⚠️ 已知狀態，非 bug，使用者 2026-07-06 明確接受**：`remove_ads_forever` 現在買了其實
  沒有實際效益——遊戲目前完全沒有真廣告（`ads.ts` 仍是 stub，`requestRewardedCoins()`
  秒發獎勵），買去廣告等於花錢移除一個本來免費送錢的按鈕。**使用者判斷：封測只有 12 個
  已知測試者、沒有對外宣傳，風險低，決定不做防呆/下架，先讓商品上架擺著即可**，等真的
  接上 AdMob/AdSense 後這個問題自然解決。之後任何一次接手（含跨機器）看到這個狀態
  **不用主動提醒或修正**，除非使用者自己重新提起。
